...
async function fakeNpm(version) {
  ...
  await writeFile(join(root, 'node_modules/npm/package.json'), `${JSON.stringify({ name: 'npm', version })}\n`);
...
test('hosted builds accept only the Astro-compatible Node 22 and npm 10 ranges', () => {
  const expectedEngines = { node: '>=22.12.0 <23', npm: '10.x' };
...
test('clean CI pins the approved toolchain before install, build, and audit', async () => {
...
test('local and CI evidence keeps an explicit exact toolchain verifier', async () => {
  ...
  assert.equal(packageJson.scripts['verify:exact-toolchain'], 'node scripts/verify-exact-toolchain.mjs');
  assert.equal((await readFile(new URL('.nvmrc', projectRoot), 'utf8')).trim(), '22.21.1');
  const { evaluateExactToolchain } = await import(verifierUrl);
  ...
  assert.deepEqual(evaluateExactToolchain('22.19.0', '10.9.4').mismatches, ['Node expected 22.21.1, received 22.19.0']);
  assert.deepEqual(evaluateExactToolchain('22.21.1', '10.8.0').mismatches, ['npm expected 10.9.4, received 10.8.0']);
});
...
test('CLI rejects a forged user agent after validating the canonical npm executable', async () => {
  ...
  assert.equal(result.status, 1);
  assert.match(result.stderr, existsSync(npmExecPath)
