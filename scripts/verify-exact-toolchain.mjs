...
export function evaluateExactToolchain(actualNode, actualNpm) {
  ...
  if (actualNode !== EXACT_NODE_VERSION) {
    mismatches.push(`Node expected ${EXACT_NODE_VERSION}, received ${actualNode}`);
  }
  if (actualNpm !== EXACT_NPM_VERSION) {
    mismatches.push(`npm expected ${EXACT_NPM_VERSION}, received ${actualNpm}`);
  }
...
function readValidatedNpmVersion() {
  if (process.env.NODE_OPTIONS || process.env.NODE_PATH) {
    refuse('Node preload and module-path variables are not allowed.');
  }
  if (process.env.npm_lifecycle_event !== 'verify:exact-toolchain'
  ...
  if (realpathSync(packageJsonPath) !== realpathSync(expectedPackageJson)) {
    refuse('npm invocation belongs to a different project.');
  }
  const nodeExecPath = realpathSync(process.execPath);
  if (realpathSync(npmNodeExecPath) !== nodeExecPath) {
    refuse('npm invocation belongs to a different Node executable.');
  }
  ...
  if (npmPackage.name !== 'npm' || typeof npmPackage.version !== 'string') {
    refuse('npm executable metadata is invalid.');
  }
  const measuredVersion = execFileSync(process.execPath, [npmExecPath, '--version'], {
    encoding: 'utf8',
    env: Object.fromEntries(
      Object.entries(process.env).filter(([name]) => !['NODE_OPTIONS', 'NODE_PATH'].includes(name)),
    ),
...
function run() {
  ...
  try {
    ...
    const reason = error instanceof InvocationRefusal
      ? error.message
      : 'Toolchain evidence could not be validated.';
