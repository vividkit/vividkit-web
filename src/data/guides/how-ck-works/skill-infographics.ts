...
const coreSkillInfographics: SkillInfographic[] = [
  // ─────────────────────────────────────────────────────────────────────────────
  // /ak:brainstorm — Solution Brainstormer
  // ─────────────────────────────────────────────────────────────────────────────
  {
    id: 'brainstorm',
    ...
    header: {
      titleEn: '/ak:brainstorm',
      titleVi: '/ak:brainstorm',
    ...
    processFlow: [
      ...
      { number: 8, titleEn: 'Report', titleVi: 'Báo cáo', descEn: 'Markdown summary via ak:project-organization (+ HTML report when --html)', descVi: 'Báo cáo markdown qua ak:project-organization (+ HTML report khi --html)' },
      { number: 9, titleEn: 'Handoff', titleVi: 'Bàn giao', descEn: 'AskUser: /ak:plan --tdd (refactor/critical) · /ak:plan (default) · end', descVi: 'AskUser: /ak:plan --tdd (refactor/critical) · /ak:plan (default) · kết thúc' },
      { number: 10, titleEn: 'Journal', titleVi: 'Nhật ký', descEn: '/ak:journal — concise technical entry', descVi: '/ak:journal — entry kỹ thuật ngắn gọn' },
    ],
    ...
    skillStack: [
      ...
      { name: 'ak:docs-seeker', type: 'skill' },
    ...
    outputFlags: [
      {
        ...
        exampleCommand: '/ak:brainstorm --html Compare auth approaches for our SaaS app',
      },
      {
        flag: '--wiki',
        ...
        exampleCommand: '/ak:brainstorm --html --wiki Compare auth approaches for our SaaS app',
      },
    ],
    ...
    promptExamples: [
      {
        ...
        command: '/ak:brainstorm Choose the best auth approach for our current SaaS app',
      ...
      },
      {
        ...
        command: '/ak:plan Create an implementation plan for the auth approach we selected in brainstorm, using the report just created as context',
      ...
      },
      {
        ...
        labelVi: 'Session mới',
        command: '/ak:plan plans/reports/brainstorm-260524-1908-auth-approach.md',
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // /ak:plan — Implementation Planner
  // ─────────────────────────────────────────────────────────────────────────────
  {
    id: 'plan',
    command: '/ak:plan',
    ...
    header: {
      titleEn: '/ak:plan',
      titleVi: '/ak:plan',
      taglineEn: 'Implementation planner with cross-plan scan, scope challenge, research phases, red-team review, and task hydration. Creates actionable phase files via the AgentKit CLI.',
      taglineVi: 'Lập kế hoạch triển khai với cross-plan scan, scope challenge, phases nghiên cứu, review red-team, và hydrate tasks. Tạo phase files có thể hành động qua AgentKit CLI.',
    },
    ...
    hardGate: {
      ...
      titleEn: 'HARD GATE',
      titleVi: 'HARD GATE',
      contentEn: 'No code implementation — /ak:plan only creates plans. CLI-owned scaffolding via `ak plan create/check/uncheck`; never hand-edit the phases table when CLI is available. Plans must live under project or global plan roots, never arbitrary user directories.',
      contentVi: 'Không triển khai code — /ak:plan chỉ tạo plans. Scaffolding do CLI sở hữu qua `ak plan create/check/uncheck`; không sửa tay bảng phases khi CLI có sẵn. Plans phải nằm dưới project hoặc global plan roots, không phải thư mục user tùy ý.',
    },
    ...
    processFlow: [
      ...
      { number: 7, titleEn: 'Plan', titleVi: 'Lập kế hoạch', descEn: 'Planner writes plan.md + phase files via AgentKit CLI', descVi: 'Planner viết plan.md + phase files qua AgentKit CLI' },
    ...
    outputFlags: [
      {
        ...
        exampleCommand: '/ak:plan --html Redesign checkout flow to reduce payment failures',
      },
      {
        flag: '--github',
        ...
        exampleCommand: '/ak:plan --github Redesign checkout flow to reduce payment failures',
      },
      {
        flag: '--wiki',
        titleEn: 'Publish to AgentWiki',
        ...
        exampleCommand: '/ak:plan --html --wiki Redesign checkout flow to reduce payment failures',
      },
    ],
    ...
    skillStack: [
      ...
      { name: 'ak:docs-seeker', type: 'skill' },
      { name: 'ak:sequential-thinking', type: 'skill' },
      { name: 'ak:project-organization', type: 'skill' },
      { name: 'ak:journal', type: 'skill' },
      { name: 'AgentKit CLI (plan create/check)', type: 'tool' },
    ...
    specialOperations: [
      {
        ...
        titleEn: 'red-team',
        titleVi: 'red-team',
        descEn: '2-4 code-reviewer agents challenge plan assumptions, find blind spots, test edge cases. Run as /ak:plan red-team {plan-path}.',
        descVi: '2-4 code-reviewer agents thách thức giả định plan, tìm điểm mù, test edge cases. Chạy /ak:plan red-team {plan-path}.',
        color: 'red',
      },
      {
        id: 'validate',
    ...
    reportOutput: {
      ...
      descEn: 'plan.md (frontmatter + phases table) • phase-XX files (canonical template) • Claude Tasks per phase • Post-plan AskUser: validate / red-team / /ak:cook {absolute-path} / end',
      descVi: 'plan.md (frontmatter + bảng phases) • file phase-XX (template chuẩn) • Claude Tasks mỗi phase • Post-plan AskUser: validate / red-team / /ak:cook {absolute-path} / end',
    },
    ...
    promptExamples: [
      {
        ...
        command: '/ak:plan Redesign checkout flow to reduce payment failures',
      ...
      },
      {
        labelEn: '--fast',
        labelVi: '--fast',
        command: '/ak:plan --fast Change the CTA copy in the Pricing section',
      ...
      },
      {
        labelEn: '--hard',
        labelVi: '--hard',
        command: '/ak:plan --hard Refactor auth flow while preserving backward compatibility',
      ...
      },
      {
        labelEn: '--deep',
        labelVi: '--deep',
        command: '/ak:plan --deep Split billing into subscription, invoice, webhook modules',
      ...
      },
      {
        labelEn: '--parallel',
        labelVi: '--parallel',
        command: '/ak:plan --parallel Optimize dashboard loading and chart rendering',
      ...
      },
      {
        labelEn: '--two',
        labelVi: '--two',
        command: '/ak:plan --two Choose an offline-first data sync architecture',
      ...
      },
      {
        labelEn: '--deep --tdd',
        labelVi: '--deep --tdd',
        command: '/ak:plan --deep --tdd Refactor the billing module',
      ...
      },
      {
        ...
        command: '/ak:plan --fast --no-tasks Update guide copy',
      ...
      // Extra flag card prompt examples — used by infographic-plan-quick-ref.astro flag cards
      {
        labelEn: 'flag-tdd',
        labelVi: 'flag-tdd',
        command: '/ak:plan --deep --tdd Refactor billing module',
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  {
    ...
    header: {
      titleEn: '/ak:cook',
      titleVi: '/ak:cook',
      taglineEn: 'Implementation engine with mandatory scout-first, exact-requirements capture, plan-gated execution, and side-effect verification. The workhorse of AgentKit.',
      taglineVi: 'Engine triển khai với scout-first bắt buộc, thu thập yêu cầu chính xác, thực thi qua plan-gate, và xác minh side-effect. Công cụ chính của AgentKit.',
    },
    ...
    processFlow: [
      ...
      { number: 10, titleEn: 'Finalize', titleVi: 'Hoàn tất', descEn: '/ak:project-management plan sync-back → docs-manager → git-manager → /ak:journal', descVi: '/ak:project-management đồng bộ plan → docs-manager → git-manager → /ak:journal' },
    ],
    ...
    expertiseAreasEn: [
      ...
      'Plan sync-back via /ak:project-management + journal documentation',
    ],
    expertiseAreasVi: [
      ...
      'Đồng bộ plan qua /ak:project-management + nhật ký kỹ thuật',
    ],
    ...
    reportOutput: {
      ...
      descEn: 'Code merged via git-manager • Tests 100% pass • code-reviewer approved (no regression, contracts intact) • plan.md + all phase-XX.md status synced via /ak:project-management • docs updated • /ak:journal entry recorded',
      descVi: 'Code merge qua git-manager • Tests 100% pass • code-reviewer duyệt (không regression, contracts còn nguyên) • plan.md + tất cả phase-XX.md đồng bộ qua /ak:project-management • docs cập nhật • /ak:journal đã ghi',
    },
    ...
    promptExamples: [
      {
        labelEn: 'Plan path',
        labelVi: 'Plan path',
        command: '/ak:cook /abs/plans/260524-auth/plan.md',
      ...
      },
      {
        ...
        command: '/ak:cook Add email/password login with httpOnly session',
      ...
      },
      {
        labelEn: '--tdd',
        labelVi: '--tdd',
        command: '/ak:cook --tdd Refactor billing calculation while preserving behavior',
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  {
    ...
    processFlow: [
      ...
      { number: 1, titleEn: 'Scout', titleVi: 'Khảo sát', descEn: 'MANDATORY — ak:scout or 2-3 parallel Explore: affected files, deps, tests, git log, patterns', descVi: 'BẮT BUỘC — ak:scout hoặc 2-3 Explore song song: file ảnh hưởng, deps, tests, git log, patterns' },
      { number: 2, titleEn: 'Diagnose', titleVi: 'Chẩn đoán', descEn: 'MANDATORY — ak:debug + ak:sequential-thinking, capture pre-fix state, evidence-based RCA, no guessing', descVi: 'BẮT BUỘC — ak:debug + ak:sequential-thinking, capture pre-fix state, RCA dựa bằng chứng, không đoán' },
      ...
      { number: 6, titleEn: 'Finalize', titleVi: 'Hoàn tất', descEn: 'MANDATORY chain: /ak:project-management → docs-manager → TaskUpdate completed → git-manager (AskUser commit) → /ak:journal', descVi: 'Chuỗi BẮT BUỘC: /ak:project-management → docs-manager → TaskUpdate completed → git-manager (AskUser commit) → /ak:journal' },
    ],
    ...
    promptExamples: [
      {
        ...
        command: '/ak:fix Login submit returns 500 in local dev',
      ...
      },
      {
        labelEn: 'High risk',
        labelVi: 'High risk',
        command: '/ak:fix --review Payment webhook double-charges paid orders',
      ...
      },
      {
        labelEn: 'CI failure',
        labelVi: 'CI failure',
        command: '/ak:fix --quick npm run build fails after latest merge',
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // /ak:team — Agent Teams Multi-Session Orchestrator
  // ─────────────────────────────────────────────────────────────────────────────
  {
    id: 'team',
    command: '/ak:team',
    ...
    processFlow: [
      { number: 1, titleEn: 'Invoke', titleVi: 'Gọi', descEn: '/ak:team <template> <context> [flags] — pick research / cook / review / debug + optional --devs/--researchers/--reviewers/--debuggers N, --delegate, --worktree', descVi: '/ak:team <template> <context> [flags] — chọn research / cook / review / debug + flag tuỳ chọn --devs/--researchers/--reviewers/--debuggers N, --delegate, --worktree' },
      ...
      { number: 4, titleEn: 'Spawn', titleVi: 'Spawn', descEn: 'Agent tool × N in parallel — model: opus, run_in_background: true, isolation: worktree (cook devs only). Each prompt includes the mandatory AgentKit Context Block', descVi: 'Agent tool × N song song — model: opus, run_in_background: true, isolation: worktree (chỉ cho cook dev). Mỗi prompt có AgentKit Context Block bắt buộc' },
      { number: 5, titleEn: 'Coordinate', titleVi: 'Điều phối', descEn: 'Poll TaskList (~60s) + SendMessage DMs to track progress — custom TaskCompleted / TeammateIdle handlers are NOT shipped by default. Teammates DM each other — adversarial in debug, lead-routed in cook', descVi: 'Theo dõi tiến độ bằng poll TaskList (~60s) + SendMessage DM — handler TaskCompleted / TeammateIdle KHÔNG ship mặc định. Teammate DM nhau — adversarial trong debug, routed qua lead trong cook' },
      ...
      { number: 7, titleEn: 'Shutdown', titleVi: 'Đóng', descEn: 'SendMessage(shutdown_request) × N → TeamDelete (NO params) → /ak:journal → report to user. Agent memory at $HOME/.claude/agent-memory/<name>/ persists separately', descVi: 'SendMessage(shutdown_request) × N → TeamDelete (KHÔNG params) → /ak:journal → báo user. Agent memory ở $HOME/.claude/agent-memory/<name>/ tồn tại độc lập' },
    ],

    corePrinciplesEn: [
      'TeamCreate-first — NEVER fall back to subagents on failure',
      'File ownership boundaries — devs must not overlap on cook tasks',
      'AgentKit Context Block in EVERY spawn prompt — teammates need it to find reports/plans',
    ...
    corePrinciplesVi: [
      ...
      'AgentKit Context Block trong MỌI spawn prompt — teammate cần để tìm reports/plans',
    ...
    expertiseAreasEn: [
      ...
      'TaskList-polling + SendMessage coordination (custom TaskCompleted / TeammateIdle handlers not shipped by default)',
    ...
    expertiseAreasVi: [
      ...
      'Điều phối qua poll TaskList + SendMessage (handler TaskCompleted / TeammateIdle không ship mặc định)',
    ...
    specialOperations: [
      {
        ...
        descEn: 'Wraps /ak:research. Default 3 angles — architecture & patterns / alternatives & trade-offs / risks & failure modes. Output: research-summary-<slug>.md.',
        descVi: 'Wrap /ak:research. Mặc định 3 góc — architecture & patterns / alternatives & trade-offs / risks & failure modes. Output: research-summary-<slug>.md.',
        color: 'sky',
      },
      {
        id: 'tpl-cook',
        ...
        descEn: 'Wraps /ak:cook. Default 4 devs (worktree-isolated) + 1 tester blocked on devs. Lead merges branches sequentially + MANDATORY docs sync eval.',
        descVi: 'Wrap /ak:cook. Mặc định 4 dev (worktree-isolated) + 1 tester chặn theo dev. Lead merge branch tuần tự + BẮT BUỘC eval docs sync.',
        color: 'amber',
      },
      {
        id: 'tpl-review',
        ...
        descEn: 'Wraps /ak:code-review. Default 3 focuses — security (OWASP) / performance / test coverage. Output: severity-rated dedupe in review-<slug>.md.',
        descVi: 'Wrap /ak:code-review. Mặc định 3 focus — security (OWASP) / performance / test coverage. Output: dedupe theo severity trong review-<slug>.md.',
        color: 'violet',
      },
      {
        id: 'tpl-debug',
        titleEn: 'debug <issue>',
        titleVi: 'debug <issue>',
        descEn: 'Wraps /ak:fix. Default 3 competing hypotheses — debuggers DM each other to disprove peers. Surviving theory = root cause in debug-<slug>.md.',
        descVi: 'Wrap /ak:fix. Mặc định 3 giả thuyết cạnh tranh — debugger DM nhau để disprove. Giả thuyết còn sống = root cause trong debug-<slug>.md.',
        color: 'rose',
      },
    ],
    ...
    reportOutput: {
      ...
      locationEn: 'plans/reports/',
      locationVi: 'plans/reports/',
      descEn: 'research → exec summary + comparative analysis + recommendations • cook → merged branches + Docs impact eval + test results • review → severity-dedupe + action items • debug → root cause + evidence chain + disproven hypotheses • All templates close with /ak:journal',
      descVi: 'research → exec summary + comparative analysis + recommendations • cook → branch đã merge + eval Docs impact + kết quả test • review → dedupe theo severity + action items • debug → root cause + evidence chain + giả thuyết đã disproven • Mọi template đều đóng bằng /ak:journal',
    },
    ...
    promptExamples: [
      {
        ...
        command: '/ak:team --research "Evaluate three caching strategies for our API gateway"',
      ...
      },
      {
        ...
        command: '/ak:team --cook "Implement auth, dashboard, and notification modules"',
      ...
      },
      {
        labelEn: 'Review team',
        labelVi: 'Review team',
        command: '/ak:team --review "Audit the payment module for security and correctness"',
      ...
      },
      {
        labelEn: 'Debug team',
        labelVi: 'Debug team',
        command: '/ak:team --debug "Investigate intermittent 502 errors in production API"',
      ...
      // Template card prompts — used by infographic-team-quick-ref.astro template cards
      {
        ...
        command: '/ak:team research Compare auth strategies for our SaaS app',
      ...
      },
      {
        labelEn: 'tpl-cook',
        labelVi: 'tpl-cook',
        command: '/ak:team cook plans/.../plan.md --devs 4 --worktree',
      ...
      },
      {
        labelEn: 'tpl-review',
        labelVi: 'tpl-review',
        command: '/ak:team review src/api --reviewers 3',
      ...
      },
      {
        labelEn: 'tpl-debug',
        labelVi: 'tpl-debug',
        command: '/ak:team debug "Webhook returns 500 intermittently" --debuggers 3',
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  {
    ...
    corePrinciplesEn: [
      ...
      'Pairs with ak:mermaidjs-v11 (syntax) and ak:tech-graph (publish-grade)',
    ],
    corePrinciplesVi: [
      ...
      'Kết hợp ak:mermaidjs-v11 (cú pháp) và ak:tech-graph (cấp publish)',
    ],
    ...
    skillStack: [
      ...
      { name: 'ak:markdown-novel-viewer', type: 'skill' },
    ...
    promptExamples: [
      {
        labelEn: '--explain',
        labelVi: '--explain',
        command: '/ak:preview --explain How does the auth middleware chain work in this repo',
      ...
      },
      {
        labelEn: '--diagram',
        labelVi: '--diagram',
        command: '/ak:preview --diagram Data flow from webhook to notification queue',
      ...
      },
      {
        labelEn: '--html --slides',
        labelVi: '--html --slides',
        command: '/ak:preview --html --slides Step-by-step guide to our deployment pipeline',
      ...
      },
      {
        labelEn: '--html --diff',
        labelVi: '--html --diff',
        command: '/ak:preview --html --diff main',
      ...
      // Flag card prompts — used by infographic-preview-quick-ref.astro flag group items
      {
        labelEn: '<file.md>',
        labelVi: '<file.md>',
        command: '/ak:preview plans/.../plan.md',
      ...
      },
      {
        labelEn: '<dir/>',
        labelVi: '<dir/>',
        command: '/ak:preview plans/260527-.../',
      ...
      },
      {
        labelEn: '<path>',
        labelVi: '<path>',
        command: '/ak:preview src/auth/middleware.ts',
      ...
      },
      {
        labelEn: '--html',
        labelVi: '--html',
        command: '/ak:preview --html --diagram "DB schema"',
      ...
      },
      {
        labelEn: '--diff [ref]',
        labelVi: '--diff [ref]',
        command: '/ak:preview --diff HEAD~3',
      ...
      },
      {
        ...
        command: '/ak:preview --plan-review plans/.../plan.md',
      ...
      },
      {
        ...
        command: '/ak:preview --recap "last week"',
      ...
      },
      {
        labelEn: '--slides',
        labelVi: '--slides',
        command: '/ak:preview --slides "onboarding flow"',
      ...
      },
      {
        labelEn: '--ascii',
        labelVi: '--ascii',
        command: '/ak:preview --ascii "folder structure"',
    ...
    deepDiveLink: {
      ...
      labelEn: 'See all AgentKit commands',
      labelVi: 'Xem tất cả lệnh AgentKit',
    },
  },

  // ─────────────────────────────────────────────────────────────────────────────
  // /ak:code-review — Adversarial Three-Stage Code Review
  // ─────────────────────────────────────────────────────────────────────────────
  {
    id: 'code-review',
    ...
    processFlow: [
      ...
      { number: 4, titleEn: 'Edge Case Scouting', titleVi: 'Scout Edge Case', descEn: 'Invoke /ak:scout with edge-case focus — 2-6 parallel Explore sub-agents scan data flows, error paths, boundary conditions. Findings feed Stage 2.', descVi: 'Gọi /ak:scout tập trung edge case — 2-6 Explore sub-agent song song quét data flows, error paths, boundary conditions. Findings nuôi Stage 2.' },
    ...
    promptExamples: [
      {
        ...
        whenEn: 'Just finished /ak:cook or /ak:fix, not yet committed.',
        whenVi: 'Vừa làm xong /ak:cook hoặc /ak:fix, chưa commit.',
      ...
      },
      {
        ...
        command: '/ak:code-review codebase parallel',
