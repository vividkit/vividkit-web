  ...
  promptExamples: [
    {
      ...
      commandVi: '/ak:use-mcp khám phá các công cụ tự động hóa trình duyệt có sẵn',
    ...
    },
    {
      ...
      commandVi: '/ak:use-mcp gọi công cụ danh sách nguồn notebook cho dự án này',
    ...
    { labelEn: 'Project MCP catalog', labelVi: 'Catalog MCP của project', command: '/ak:use-mcp discover tools from .claude/.mcp.json for this project',
      commandVi: '/ak:use-mcp khám phá các công cụ từ .claude/.mcp.json cho dự án này', whenEn: 'The runtime does not expose a server but the project MCP config exists.', whenVi: 'Runtime không hiện server nhưng project có MCP config.', expectedEn: 'Uses the scripted catalog path against .claude/.mcp.json without copying credentials across runtimes.', expectedVi: 'Dùng đường catalog script với .claude/.mcp.json, không chép credential sang runtime khác.' }
  ],
  specialOperations: [
    {
      id: "runtime-native",
