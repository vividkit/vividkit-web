...
const data: SkillInfographic = {
  ...
  promptExamples: [
    { labelEn: 'Find feature files', labelVi: 'Tìm file feature', command: '/ak:scout checkout flow',
      commandVi: '/ak:scout luồng thanh toán', whenEn: 'You need to find where a cross-file feature lives before editing.', whenVi: 'Khi cần tìm một feature nằm ở đâu trong nhiều file trước khi sửa.', expectedEn: 'Relevant files with roles, relationships, unresolved questions, and evidence for the next implementer.', expectedVi: 'Các file liên quan kèm vai trò, quan hệ và câu hỏi còn mở.', recommended: true },
    { labelEn: 'Extension scope', labelVi: 'Scope theo đuôi file', command: '/ak:scout email templates ts',
      commandVi: '/ak:scout email mẫu ts', whenEn: 'You need scoped discovery around a target and file extension.', whenVi: 'Khi cần discovery có scope quanh mục tiêu và loại file.', expectedEn: 'Narrowed file map for the requested target and extension, with likely entry points identified.', expectedVi: 'File map đã thu hẹp theo mục tiêu và đuôi file được yêu cầu.' },
    { labelEn: 'External scouting', labelVi: 'Scouting ngoài', command: '/ak:scout ext authentication module',
      commandVi: '/ak:scout ext mô-đun xác thực', whenEn: 'Native search is not enough and user-permitted external probes are appropriate.', whenVi: 'Khi native search không đủ và probe ngoài được phép là phù hợp.', expectedEn: 'External-probe-backed scout report with local evidence and clear confidence boundaries for follow-up work.', expectedVi: 'Scout report có hỗ trợ từ probe bên ngoài kèm evidence local và ranh giới confidence rõ.' },
    { labelEn: 'Ultra verifier', labelVi: 'Ultra verifier', command: '/ak:scout checkout state --ultra',
      commandVi: '/ak:scout trạng thái thanh toán --ultra', whenEn: 'You need a fail-closed best-of-5 verification pass.', whenVi: 'Khi cần lượt verify best-of-5 và fail-closed.', expectedEn: 'Evidence-validated, deduplicated union of findings across candidates with conflicts called out clearly.', expectedVi: 'Union finding đã validate bằng evidence, khử trùng lặp và nêu rõ xung đột giữa các ứng viên.' },
  ],
  reportOutput: {
