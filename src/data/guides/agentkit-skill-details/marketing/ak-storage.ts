...
const data: SkillInfographic = {
  ...
  promptExamples: [
    { labelEn: 'Upload asset', labelVi: 'Upload asset', command: '/ak:storage "Upload assets/images/launch-hero.png to campaign/launch/hero.png only after approval; return the URL if configured"',
      commandVi: '/ak:storage "Tải lên assets/images/launch-hero.png tới campaign/launch/hero.png chỉ sau khi được phê duyệt; trả về URL nếu đã cấu hình"', whenEn: 'You need a generated marketing asset hosted remotely.', whenVi: 'Khi cần host remote một asset marketing đã tạo.', expectedEn: 'The client reads the local file, sends PutObject when approved, and returns a public URL or a graceful unconfigured fallback instead of throwing.', expectedVi: 'Client đọc file local, gửi PutObject khi đã duyệt, rồi trả URL public hoặc lỗi fallback nhẹ khi chưa cấu hình, không throw.', recommended: true },
    { labelEn: 'Download object', labelVi: 'Download object', command: '/ak:storage "Download designs/pricing-deck.html to assets/slides/pricing-deck.html after confirming the key"',
      commandVi: '/ak:storage "Tải xuống designs/pricing-deck.html tới assets/slides/pricing-deck.html sau khi xác nhận khóa"', whenEn: 'You need one approved remote object written to a local path.', whenVi: 'Khi cần ghi đúng một object remote đã duyệt ra path local.', expectedEn: 'The download operation creates parent directories and writes the object bytes to the requested local path, or reports a missing-key or config error without crashing.', expectedVi: 'Thao tác download tạo thư mục cha và ghi byte object ra path local đã nêu, hoặc báo thiếu key/config mà không crash.' },
    { labelEn: 'List folder', labelVi: 'Liệt kê thư mục', command: '/ak:storage "List approved remote keys under designs/ without deleting or uploading anything"',
      commandVi: '/ak:storage "Liệt kê các khóa từ xa đã được phê duyệt dưới designs/ mà không xóa hoặc tải lên bất kỳ thứ gì"', whenEn: 'You need to inspect remote asset keys under a prefix.', whenVi: 'Khi cần xem các object key remote dưới một prefix.', expectedEn: 'ListObjectsV2 returns a bounded object list for that prefix, or a fallback status when credentials or the SDK are missing.', expectedVi: 'ListObjectsV2 trả danh sách object giới hạn theo prefix, hoặc trạng thái fallback khi thiếu credential hay SDK.' },
    { labelEn: 'Remove key', labelVi: 'Xóa key', command: '/ak:storage "Remove exactly the approved key drafts/old-banner.png and no other objects"',
      commandVi: '/ak:storage "Xóa chính xác khóa đã được phê duyệt drafts/old-banner.png và không xóa đối tượng nào khác"', whenEn: 'You must delete one approved object and nothing else.', whenVi: 'Khi phải xóa đúng một object đã duyệt, không xóa thêm.', expectedEn: 'The remove operation deletes that single approved key and reports success or a bounded failure without walking the bucket.', expectedVi: 'Thao tác remove xóa đúng một key đã duyệt và báo thành công hoặc lỗi có giới hạn, không quét cả bucket.' },
    { labelEn: 'Exists check', labelVi: 'Kiểm exists', command: '/ak:storage "Check whether campaign/launch/hero.png exists with HeadObject and do not download it"',
      commandVi: '/ak:storage "Kiểm tra xem campaign/launch/hero.png có tồn tại bằng HeadObject và không tải xuống"', whenEn: 'You only need to know if one remote key is present.', whenVi: 'Khi chỉ cần biết một remote key có tồn tại hay không.', expectedEn: 'HeadObject returns an existence result for that key, distinguishing missing objects from auth or config failures.', expectedVi: 'HeadObject trả kết quả tồn tại cho key đó, phân biệt object thiếu với lỗi auth hoặc config.' },
    { labelEn: 'Public URL', labelVi: 'URL public', command: '/ak:storage "Use getPublicUrl for designs/pricing-deck.html without making a provider request"',
      commandVi: '/ak:storage "Dùng getPublicUrl cho designs/pricing-deck.html mà không gửi yêu cầu tới nhà cung cấp"', whenEn: 'You need a shareable URL for an already uploaded asset.', whenVi: 'Khi cần URL chia sẻ cho asset đã upload.', expectedEn: 'getPublicUrl builds a custom-domain or provider URL from config without sending a request to the bucket.', expectedVi: 'getPublicUrl dựng URL custom domain hoặc provider từ config mà không gửi request tới bucket.' },
    { labelEn: 'Sync assets', labelVi: 'Sync asset', command: '/ak:storage "Preview syncing assets/slides/ to slides/ as a dry run; show every object key and wait before upload"',
      commandVi: '/ak:storage "Xem trước đồng bộ assets/slides/ tới slides/ ở chế độ dry run; hiển thị mọi khóa đối tượng và chờ trước khi tải lên"', whenEn: 'You want a local asset folder mirrored to remote storage.', whenVi: 'Khi muốn mirror một thư mục asset local lên remote storage.', expectedEn: 'The sync dry run lists every local-file-to-object mapping and waits for approval before any PutObject.', expectedVi: 'Dry run sync liệt kê mọi mapping file-local sang object và chờ duyệt trước khi PutObject.' },
  ],
};
