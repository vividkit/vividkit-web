...
const data: SkillInfographic = {
  ...
  header: {
    ...
    taglineEn: 'Marketing and product-discovery playbook orchestrator with dependency graph routing, template management, goal tracking, quality gates, smart suggestions, and stateful manifest updates.',
    taglineVi: 'Bộ điều phối playbook marketing và khám phá sản phẩm với dependency graph, template, goal tracking, quality gate, gợi ý thông minh và cập nhật manifest có trạng thái.',
  },
  hardGate: {
    type: 'critical',
  ...
  workflowModes: [
    ...
    { flag: 'templates [--browse]', modeEn: 'Browse bundled templates such as product-discovery, product-hunt-launch, content-engine, campaign-sprint, and saas-launch.', modeVi: 'Duyệt template có sẵn như product-discovery, product-hunt-launch, content-engine, campaign-sprint và saas-launch.', research: 'Template catalog', redTeam: 'Unsupported gate', validation: 'Schema' },
  ],
  promptExamples: [
    { labelEn: 'Product discovery', labelVi: 'Khám phá sản phẩm', command: '/ak:play create idea-check --template product-discovery',
      commandVi: '/ak:play create idea-check --template product-discovery', whenEn: 'A founder or product owner needs the discovery playbook before a campaign playbook.', whenVi: 'Khi founder hoặc product owner cần playbook khám phá trước playbook chiến dịch.', expectedEn: 'Creates a product-discovery playbook whose publish, send, and spend steps stay behind a founder-decision or human-approval gate.', expectedVi: 'Tạo playbook product-discovery mà các bước publish, gửi hoặc chi tiêu vẫn nằm sau cổng quyết định của founder hoặc phê duyệt của con người.' },
    { labelEn: 'Create playbook', labelVi: 'Tạo playbook', command: '/ak:play create saas-launch --template saas-launch',
      commandVi: '/ak:play create saas-launch --template saas-launch', whenEn: 'You need a reproducible marketing playbook from a bundled template.', whenVi: 'Khi cần playbook marketing lặp lại được từ template có sẵn.', expectedEn: 'This manifest-backed playbook outlines the required steps, goals, dependencies, and the template version used throughout the process.', expectedVi: 'Playbook có manifest với step, goal, dependency và version template, giúp đội ngũ theo dõi toàn bộ quy trình đã định nghĩa.', recommended: true },
    { labelEn: 'Next action', labelVi: 'Hành động tiếp', command: '/ak:play next saas-launch',
      commandVi: '/ak:play next saas-launch', whenEn: 'You want the highest-impact ready step instead of scanning the whole plan.', whenVi: 'Khi muốn bước ready có impact cao nhất thay vì tự rà toàn bộ plan.', expectedEn: 'It surfaces the top suggested actions derived from graph readiness signals and the identified goal gaps.', expectedVi: 'Các hành động gợi ý hàng đầu từ graph readiness và goal gap.' },
    { labelEn: 'Status dashboard', labelVi: 'Dashboard trạng thái', command: '/ak:play status saas-launch',
      commandVi: '/ak:play status saas-launch', whenEn: 'You need current progress, blocked steps, stale work, goals, and outputs.', whenVi: 'Khi cần tiến độ hiện tại, step bị block, việc stale, goal và output.', expectedEn: 'The playbook status dashboard gives teams a single view of current progress, blockers, and overall readiness.', expectedVi: 'Dashboard trạng thái playbook hiển thị tiến độ hiện tại, các bước đã xong và những điểm cần chú ý.' },
    { labelEn: 'Gate decision', labelVi: 'Quyết định gate', command: '/ak:play gate saas-launch pricing-page approve',
      commandVi: '/ak:play gate saas-launch pricing-page approve', whenEn: 'A human decision gate is pending and needs an explicit decision.', whenVi: 'Khi human gate đang chờ và cần quyết định rõ.', expectedEn: 'After the manifest gate status is updated, the system recalculates which step is next ready to run.', expectedVi: 'Manifest cập nhật trạng thái gate và tính lại step ready tiếp theo.' },
    { labelEn: 'Goal pull', labelVi: 'Kéo goal', command: '/ak:play goals pull',
      commandVi: '/ak:play goals pull', whenEn: 'You need current metric progress using available analytics bridges or manual fallback.', whenVi: 'Khi cần cập nhật metric hiện tại qua analytics bridge hoặc fallback nhập tay.', expectedEn: 'The view shows goal values alongside their trends and the smart suggestions generated from those signals.', expectedVi: 'Màn hình này trình bày giá trị goal, trend tương ứng và các gợi ý thông minh dựa trên dữ liệu đó.' },
    { labelEn: 'Browse templates', labelVi: 'Duyệt template', command: '/ak:play templates --browse',
      commandVi: '/ak:play templates --browse', whenEn: 'You need to inspect bundled playbook templates before creating one.', whenVi: 'Khi cần xem template playbook có sẵn trước khi tạo mới.', expectedEn: 'Lists bundled templates with ids and descriptions and does not create or mutate a playbook manifest.', expectedVi: 'Liệt kê template có sẵn với id và mô tả, không tạo hoặc sửa manifest playbook.' },
  ],
  skillStack: [
