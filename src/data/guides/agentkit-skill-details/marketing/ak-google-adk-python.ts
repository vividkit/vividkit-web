...
const data: SkillInfographic = {
  ...
  promptExamples: [
    { labelEn: 'Multi-agent system', labelVi: 'Hệ multi-agent', command: '/ak:google-adk-python multi-agent support bot with MCP tools', whenEn: 'You need ADK architecture for specialized sub-agents and tools.', whenVi: 'Cần kiến trúc ADK cho nhiều sub-agent chuyên trách và tool.', expectedEn: 'Chooses agent types, package convention, MCPToolset wiring, state, and eval path.', expectedVi: 'Chọn loại agent, convention package, cách nối MCPToolset, state và đường evaluation.', recommended: true },
    { labelEn: 'A2A integration', labelVi: 'Tích hợp A2A', command: '/ak:google-adk-python A2A remote agent integration', whenEn: 'Remote agents should communicate through A2A protocol.', whenVi: 'Các agent từ xa cần giao tiếp qua giao thức A2A.', expectedEn: 'Loads the multi-agent and A2A reference and outlines RemoteA2aAgent usage.', expectedVi: 'Nạp tham chiếu multi-agent và A2A rồi phác thảo cách dùng RemoteA2aAgent.' },
    { labelEn: 'Vertex deployment', labelVi: 'Deploy Vertex', command: '/ak:google-adk-python deploy agent to Vertex AI Agent Engine', whenEn: 'An ADK agent is ready for managed deployment.', whenVi: 'Agent ADK đã sẵn sàng triển khai dạng managed.', expectedEn: 'Uses the deployment reference and includes evaluation before rollout.', expectedVi: 'Dùng tài liệu deploy và đưa evaluation vào trước khi rollout.' },
  ],
  skillStack: [
