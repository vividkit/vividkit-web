...
const data: SkillInfographic = {
  ...
  promptExamples: [
    { labelEn: 'Default fix', labelVi: 'Sửa mặc định', command: '/ak:fix "checkout submit throws TypeError" --auto',
      commandVi: '/ak:fix "checkout submit ném TypeError" --auto', whenEn: 'A concrete bug can be handled autonomously.', whenVi: 'Có lỗi cụ thể có thể để skill tự xử lý.', expectedEn: 'Frames intent, scouts the failing area, diagnoses root cause, implements the fix, verifies, and finalizes.', expectedVi: 'Đóng khung, khảo sát, chẩn đoán, sửa, kiểm chứng và kết thúc.', recommended: true },
    { labelEn: 'Trivial issue', labelVi: 'Lỗi nhỏ', command: '/ak:fix "eslint no-unused-vars in src/app.ts" --quick',
      commandVi: '/ak:fix "eslint no-unused-vars trong src/app.ts" --quick', whenEn: 'The issue is a simple lint or type failure.', whenVi: 'Lỗi là lint hoặc type đơn giản.', expectedEn: 'Uses the quick scout to diagnose and repair the issue without skipping focused verification afterward.', expectedVi: 'Dùng khảo sát nhanh để chẩn đoán và sửa nhưng vẫn kiểm chứng.' },
    { labelEn: 'Human-in-loop', labelVi: 'Có duyệt từng bước', command: '/ak:fix "production payment regression" --review',
      commandVi: '/ak:fix "hồi quy thanh toán production" --review', whenEn: 'The bug touches critical or production-sensitive code.', whenVi: 'Lỗi chạm vào mã quan trọng hoặc nhạy cảm production.', expectedEn: 'Pauses at review gates before risky decisions so the user can approve material scope changes.', expectedVi: 'Dừng ở các cổng duyệt trước quyết định rủi ro để người dùng duyệt thay đổi phạm vi quan trọng.' },
    { labelEn: 'Independent issues', labelVi: 'Nhiều lỗi độc lập', command: '/ak:fix "CI failures in auth and billing" --parallel',
      commandVi: '/ak:fix "lỗi CI trong auth và billing" --parallel', whenEn: 'Two or more failures can be owned independently.', whenVi: 'Có từ hai lỗi trở lên có thể chia chủ sở hữu độc lập.', expectedEn: 'Routes each independent issue to parallel fullstack-developer ownership with clear boundaries and merge expectations.', expectedVi: 'Định tuyến từng lỗi độc lập cho các nhánh xử lý song song với ranh giới rõ ràng.' },
    { labelEn: 'Ultra verifier', labelVi: 'Verifier ultra', command: '/ak:fix "race in checkout retry" --ultra',
      commandVi: '/ak:fix "race trong checkout retry" --ultra', whenEn: 'Fix selection is high-stakes and should use a verifier pass.', whenVi: 'Khi chọn hướng sửa có rủi ro cao và nên dùng lượt verifier.', expectedEn: 'Runs one diagnosis packet, five read-only candidate plans, verifier selection, then a single implementation path.', expectedVi: 'Chạy một gói chẩn đoán, năm plan ứng viên chỉ đọc, verifier chọn, rồi một đường triển khai duy nhất.' },
    { labelEn: 'Advisory supervision', labelVi: 'Giám sát cố vấn', command: '/ak:fix "production payment regression" --advice',
      commandVi: '/ak:fix "hồi quy thanh toán production" --advice', whenEn: 'You want kongming counsel after phases without letting advice edit code.', whenVi: 'Khi muốn tư vấn kongming sau các phase mà advice không được sửa code.', expectedEn: 'Keeps the evidence-first fix path, adds advisory checkpoints, and still requires root-cause proof plus verification.', expectedVi: 'Giữ đường sửa theo bằng chứng, thêm checkpoint cố vấn và vẫn bắt buộc chứng minh nguyên nhân gốc cùng kiểm chứng.' },
  ],
  skillStack: [
