...
const data: SkillInfographic = {
  ...
  outputFlags: [
    { flag: '--html', titleEn: 'Self-contained HTML', titleVi: 'HTML độc lập', descEn: 'Switch generation output to a single inline HTML file that opens directly in a browser.', descVi: 'Chuyển output tạo mới sang một file HTML inline, mở trực tiếp trong browser.', exampleCommand: '/ak:preview --html --diagram "Landing page funnel"',
          exampleCommandVi: '/ak:preview --html --diagram "Phễu trang đích"' },
    { flag: '--no-editorial-visuals', titleEn: 'Disable editorial engines', titleVi: 'Tắt engine editorial', descEn: 'Disable both additive HTML editorial visual engines and use fallback rendering.', descVi: 'Tắt cả hai engine editorial visual bổ sung cho HTML và dùng renderer fallback.', exampleCommand: '/ak:preview --html --diagram "Launch flow" --no-editorial-visuals',
          exampleCommandVi: '/ak:preview --html --diagram "Luồng ra mắt" --no-editorial-visuals' },
    { flag: '--no-antv', titleEn: 'Disable AntV Infographic', titleVi: 'Tắt AntV Infographic', descEn: 'Disable only the AntV Infographic layer while leaving other eligible visual engines available.', descVi: 'Chỉ tắt lớp AntV Infographic trong khi vẫn cho phép engine visual phù hợp khác.', exampleCommand: '/ak:preview --html --recap "last week" --no-antv',
          exampleCommandVi: '/ak:preview --html --recap "tuần trước" --no-antv' },
    { flag: '--no-diagram-design', titleEn: 'Disable diagram-design', titleVi: 'Tắt diagram-design', descEn: 'Disable only the diagram-design layer and fall back to Mermaid or Chart.js where appropriate.', descVi: 'Chỉ tắt lớp diagram-design và fallback sang Mermaid hoặc Chart.js khi phù hợp.', exampleCommand: '/ak:preview --html --diagram "System map" --no-diagram-design',
          exampleCommandVi: '/ak:preview --html --diagram "Bản đồ hệ thống" --no-diagram-design' },
  ],
  promptExamples: [
    { labelEn: 'View file', labelVi: 'Xem file', command: '/ak:preview assets/reports/seo/launch-audit.md',
      commandVi: '/ak:preview assets/reports/seo/launch-audit.md', whenEn: 'You want to view an existing Markdown or file output.', whenVi: 'Khi muốn xem Markdown hoặc file output đã có.', expectedEn: 'The requested file opens in the preview viewer so the user can inspect rendered output immediately.', expectedVi: 'File đã yêu cầu mở trong trình preview để người dùng xem ngay kết quả render.', recommended: true },
    { labelEn: 'Explain', labelVi: 'Giải thích', command: '/ak:preview --explain How our referral funnel works',
      commandVi: '/ak:preview --explain Cách phễu giới thiệu của chúng ta hoạt động', whenEn: 'You need a visual explanation of a concept or system.', whenVi: 'Khi cần giải thích trực quan một concept hoặc hệ thống.', expectedEn: 'Markdown explanation includes visual structure so the reader can follow sections, relationships, and decisions.', expectedVi: 'Giải thích Markdown có cấu trúc visual để người đọc theo dõi phần, quan hệ và quyết định.' },
    { labelEn: 'HTML slides', labelVi: 'Slide HTML', command: '/ak:preview --html --slides Launch narrative for investors',
      commandVi: '/ak:preview --html --slides Câu chuyện ra mắt dành cho nhà đầu tư', whenEn: 'You need a browser-ready slide deck.', whenVi: 'Khi cần slide deck mở được ngay trên browser.', expectedEn: 'Self-contained HTML slide deck is generated with readable sections, navigation, and portable assets.', expectedVi: 'Slide deck HTML độc lập được tạo với các phần dễ đọc, điều hướng và asset portable.' },
    { labelEn: 'Plan review', labelVi: 'Review plan', command: '/ak:preview --plan-review plans/launch-plan.md',
      commandVi: '/ak:preview --plan-review plans/launch-plan.md', whenEn: 'You want to compare a plan with current files.', whenVi: 'Khi muốn so sánh plan với file hiện tại.', expectedEn: 'HTML plan-vs-codebase comparison highlights alignment, gaps, and recommended next changes in one page.', expectedVi: 'Trang HTML so sánh plan với codebase nêu rõ điểm khớp, khoảng trống và thay đổi tiếp theo.' },
    { labelEn: 'Stop server', labelVi: 'Dừng server', command: '/ak:preview --stop',
      commandVi: '/ak:preview --stop', whenEn: 'You need to stop the running preview server.', whenVi: 'Khi cần dừng preview server đang chạy.', expectedEn: 'Preview server is stopped cleanly so no background viewer process remains after inspection.', expectedVi: 'Preview server được dừng sạch để không còn tiến trình viewer nền sau khi kiểm tra.' },
    { labelEn: 'Diagram', labelVi: 'Sơ đồ', command: '/ak:preview --diagram referral funnel stages',
      commandVi: '/ak:preview --diagram các giai đoạn phễu giới thiệu', whenEn: 'You need a focused diagram instead of opening an existing file.', whenVi: 'Khi cần sơ đồ tập trung thay vì mở file có sẵn.', expectedEn: 'Generates a diagram artifact for the topic and reports the output path without rewriting source files.', expectedVi: 'Sinh artifact sơ đồ cho topic và báo path output mà không viết lại file nguồn.' },
    { labelEn: 'ASCII diagram', labelVi: 'Sơ đồ ASCII', command: '/ak:preview --ascii referral funnel stages',
      commandVi: '/ak:preview --ascii các giai đoạn phễu giới thiệu', whenEn: 'The diagram must stay readable in a terminal without HTML.', whenVi: 'Khi sơ đồ phải đọc được trong terminal, không cần HTML.', expectedEn: 'Writes a terminal-readable ASCII diagram for the topic and skips the HTML preview path.', expectedVi: 'Ghi sơ đồ ASCII đọc được trên terminal cho topic và bỏ đường preview HTML.' },
    { labelEn: 'Diff review', labelVi: 'Review diff', command: '/ak:preview --diff main',
      commandVi: '/ak:preview --diff main', whenEn: 'Current git changes need a visual review before handoff.', whenVi: 'Khi thay đổi git hiện tại cần review trực quan trước bàn giao.', expectedEn: 'Detects the diff against main and creates a review artifact covering changed files and decision context.', expectedVi: 'Xác định diff so với main và tạo artifact review gồm file đổi và context quyết định.' },
    { labelEn: 'Session recap', labelVi: 'Recap phiên', command: '/ak:preview --recap 7d',
      commandVi: '/ak:preview --recap 7d', whenEn: 'You need a visual recap of recent work over a timeframe.', whenVi: 'Khi cần recap trực quan công việc gần đây theo khung thời gian.', expectedEn: 'Collects the requested timeframe, generates a recap visual, and reports the artifact path.', expectedVi: 'Thu thập khung thời gian đã yêu cầu, sinh visual recap và báo path artifact.' },
  ],
};
