...
const data: SkillInfographic = {
  ...
  invocation: {
    syntax: '/ak:mintlify [task] [path]',
    arguments: [
      { token: '[task]', titleEn: 'Docs task', titleVi: 'Việc docs', descEn: 'Natural-language outcome, constraints, allowed files, validation commands, and stop points. It should not grant hosted account, dashboard, DNS, Git, preview publication, or deployment permission by itself.', descVi: 'Outcome, ràng buộc, file được phép đổi, lệnh validate và điểm dừng bằng ngôn ngữ tự nhiên. Nội dung này không tự cấp quyền thao tác hosted account, dashboard, DNS, Git, publish preview hoặc deploy.', required: true, exampleCommand: '/ak:mintlify "In ./docs-site, add an Authentication group, run mint validate and mint broken-links, and stop before deployment."',
          exampleCommandVi: '/ak:mintlify "Trong ./docs-site, add một group Authentication, chạy mint validate và mint broken-links, và stop trước deployment."' },
      { token: '[path]', titleEn: 'Docs root', titleVi: 'Thư mục docs', descEn: 'Optional repository path to the Mintlify project or docs root. Include it when the target is not obvious from the current workspace.', descVi: 'Đường dẫn tùy chọn tới project Mintlify hoặc thư mục docs. Nên ghi khi target không rõ từ workspace hiện tại.', exampleCommand: '/ak:mintlify "Update API navigation" ./docs-site',
          exampleCommandVi: '/ak:mintlify "Update điều hướng API" ./docs-site' },
    ],
  },
  promptExamples: [
    { labelEn: 'Docs structure', labelVi: 'Cấu trúc docs', command: '/ak:mintlify reorganize API docs navigation',
      commandVi: '/ak:mintlify tổ chức lại điều hướng docs API', whenEn: 'Use when Mintlify docs site structure, navigation, or page grouping needs work.', whenVi: 'Dùng khi cấu trúc site docs Mintlify, navigation hoặc nhóm trang cần chỉnh.', expectedEn: 'Reworks docs.json navigation using supported groups, tabs, anchors, products, versions, or language patterns while preserving page frontmatter.', expectedVi: 'Sắp xếp lại navigation docs.json bằng group, tab, anchor, product, version hoặc pattern ngôn ngữ được hỗ trợ, đồng thời giữ frontmatter trang.', recommended: true },
    { labelEn: 'MDX page', labelVi: 'Trang MDX', command: '/ak:mintlify add quickstart page with Steps and CodeGroup',
      commandVi: '/ak:mintlify thêm trang quickstart với Steps và CodeGroup', whenEn: 'Use when adding or improving Mintlify MDX content pages.', whenVi: 'Dùng khi thêm hoặc cải thiện trang nội dung MDX của Mintlify.', expectedEn: 'Creates or updates an MDX page with correct frontmatter and Mintlify components such as Steps, CodeGroup, Note, callouts, or diagrams.', expectedVi: 'Tạo hoặc cập nhật trang MDX với frontmatter đúng và component Mintlify như Steps, CodeGroup, Note, callout hoặc sơ đồ.' },
    { labelEn: 'API docs', labelVi: 'Docs API', command: '/ak:mintlify connect OpenAPI spec to docs site',
      commandVi: '/ak:mintlify kết nối OpenAPI spec với trang docs', whenEn: 'Use when Mintlify interactive API documentation is needed.', whenVi: 'Dùng khi cần tài liệu API tương tác trong Mintlify.', expectedEn: 'Connects OpenAPI or AsyncAPI specs to Mintlify API documentation, including playground behavior, multi-language examples, and openapi-check validation.', expectedVi: 'Kết nối spec OpenAPI hoặc AsyncAPI vào tài liệu API Mintlify, gồm hành vi playground, ví dụ đa ngôn ngữ và validate bằng openapi-check.' },
    { labelEn: 'Local checks', labelVi: 'Kiểm tra cục bộ', command: '/ak:mintlify run local validation for docs.json and links',
      commandVi: '/ak:mintlify chạy validation cục bộ cho docs.json và các liên kết', whenEn: 'Use when a Mintlify docs site needs preview, configuration, links, accessibility, or API-spec checks before shipping.', whenVi: 'Dùng khi site docs Mintlify cần preview, kiểm tra cấu hình, link, accessibility hoặc spec API trước khi giao.', expectedEn: 'Runs the relevant Mintlify CLI path such as mint dev, mint validate, mint broken-links, mint a11y, or mint openapi-check and reports fixes needed.', expectedVi: 'Chạy luồng Mintlify CLI phù hợp như mint dev, mint validate, mint broken-links, mint a11y hoặc mint openapi-check và báo phần cần sửa.' },
  ],
  skillStack: [
