...
const data: SkillInfographic = {
  ...
  workflowModes: [
    { flag: '--export png|svg', modeEn: 'Static export', modeVi: 'Xuất tĩnh', research: 'file-workflow', redTeam: 'Check clipping and routing', validation: 'PNG or SVG written', cookFlag: '--export <png|svg>' },
    { flag: '--live', modeEn: 'MCP canvas', modeVi: 'Canvas MCP', research: 'mcp-workflow', redTeam: 'Max 2 self-critique iterations', validation: 'Live canvas screenshot', cookFlag: '--live' },
  ],
  invocation: {
    syntax: '/ak:excalidraw [diagram description|path] [--export <png|svg>] [--live]',
    arguments: [
      {
        token: '[diagram description|path]',
        titleEn: 'Description or path',
        titleVi: 'Mô tả hoặc đường dẫn',
        descEn: 'Optional natural-language diagram request or a path to an existing .excalidraw file. Omit it only when continuing an already open live canvas.',
        descVi: 'Yêu cầu sơ đồ bằng ngôn ngữ tự nhiên hoặc path tới file .excalidraw có sẵn, đều tùy chọn. Chỉ bỏ trống khi đang tiếp tục canvas live đã mở.',
        required: false,
        exampleCommand: '/ak:excalidraw draw our queue-based ingestion architecture',
        exampleCommandVi: '/ak:excalidraw draw our queue-based ingestion architecture',
      },
    ],
    options: [
      {
        token: '--export <png|svg>',
        titleEn: 'Export format',
        titleVi: 'Định dạng xuất',
        descEn: 'Write a PNG or SVG snapshot in addition to the editable Excalidraw source or live canvas. Export is review output, not a replacement for the editable diagram.',
        descVi: 'Ghi snapshot PNG hoặc SVG ngoài source Excalidraw chỉnh sửa được hoặc canvas live. Export là output để review, không thay sơ đồ chỉnh sửa được.',
        exampleCommand: '/ak:excalidraw visualize the onboarding workflow --export png',
        exampleCommandVi: '/ak:excalidraw visualize the onboarding workflow --export png',
      },
      {
        token: '--live',
        titleEn: 'Live MCP canvas',
        titleVi: 'Canvas MCP live',
        descEn: 'Prefer the live Excalidraw MCP canvas when that backend is available. If the canvas probe fails, fall back to .excalidraw JSON plus Playwright rendering.',
        descVi: 'Ưu tiên canvas Excalidraw MCP live khi backend đó có sẵn. Nếu probe canvas thất bại, fallback sang JSON .excalidraw và render bằng Playwright.',
        exampleCommand: '/ak:excalidraw draw our queue-based ingestion architecture --live',
        exampleCommandVi: '/ak:excalidraw draw our queue-based ingestion architecture --live',
      },
    ],
  },
  specialOperations: [
  ...
  promptExamples: [
    {
      labelEn: 'Architecture canvas',
      labelVi: 'Canvas kiến trúc',
      command: '/ak:excalidraw draw our queue-based ingestion architecture --live',
      commandVi: '/ak:excalidraw draw our queue-based ingestion architecture --live',
      whenEn: 'The user needs an editable live architecture canvas, not a static editorial render.',
      whenVi: 'Người dùng cần canvas kiến trúc live có thể chỉnh sửa, không phải bản render editorial tĩnh.',
      expectedEn: 'Probes the live MCP canvas first, researches concrete components and events for the technical diagram, maps those facts into visual patterns, then renders and repairs the editable Excalidraw canvas.',
      expectedVi: 'Probe canvas MCP live trước, nghiên cứu component và event cụ thể cho sơ đồ kỹ thuật, chuyển các fact đó thành visual pattern, rồi render và sửa canvas Excalidraw có thể chỉnh sửa.',
      recommended: true,
    },
    {
      labelEn: 'Auto repo map',
      labelVi: 'Tự map repo',
      command: '/ak:excalidraw diagram this repo',
      commandVi: '/ak:excalidraw diagram this repo',
      whenEn: 'The request is zero-config codebase visualization.',
      whenVi: 'Yêu cầu là trực quan hóa codebase không cần cấu hình.',
      expectedEn: 'Detects the project/framework, discovers components and connections within the tool-call limits, asks only for an unresolved material layout decision, then generates an overview capped at 12 components and 20 arrows.',
      expectedVi: 'Nhận diện project/framework, tìm component và kết nối trong giới hạn tool-call, chỉ hỏi khi còn quyết định layout/nội dung chưa giải được, rồi tạo overview giới hạn 12 component và 20 mũi tên.',
    },
    {
      labelEn: 'Workflow PNG export',
      labelVi: 'Xuất workflow PNG',
      command: '/ak:excalidraw visualize the onboarding workflow --export png',
      commandVi: '/ak:excalidraw visualize the onboarding workflow --export png',
      whenEn: 'A workflow needs an editable diagram plus a PNG snapshot for review.',
      whenVi: 'Workflow cần sơ đồ có thể chỉnh sửa kèm snapshot PNG để review.',
      expectedEn: 'Chooses a timeline, cycle, or phase-break structure instead of uniform cards, creates the Excalidraw source or canvas, visually checks clipping and arrow routing, then writes the requested PNG export.',
      expectedVi: 'Chọn cấu trúc timeline, cycle hoặc phase-break thay vì card đồng dạng, tạo source Excalidraw hoặc canvas, kiểm tra clipping và hướng arrow, rồi ghi file PNG đã yêu cầu.',
    },
    {
      labelEn: 'SVG export',
      labelVi: 'Xuất SVG',
      command: '/ak:excalidraw visualize the onboarding workflow --export svg',
      commandVi: '/ak:excalidraw visualize the onboarding workflow --export svg',
      whenEn: 'The same workflow needs a vector snapshot rather than a raster PNG.',
      whenVi: 'Cùng workflow đó cần snapshot vector thay vì PNG raster.',
      expectedEn: 'Keeps the editable Excalidraw source, visually checks overlaps, spacing, and arrow routing, then writes an SVG export for review without replacing the editable diagram.',
      expectedVi: 'Giữ source Excalidraw chỉnh sửa được, kiểm tra overlap, spacing và hướng arrow, rồi ghi file SVG để review mà không thay sơ đồ chỉnh sửa được.',
    },
  ],
};
