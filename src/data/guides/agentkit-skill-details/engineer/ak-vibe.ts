...
const data: SkillInfographic = {
  ...
  processFlow: [
    { number: 1, titleEn: 'Parse request', titleVi: 'Phân tích yêu cầu', descEn: 'Strip pipeline flags, resolve mode as both > beta > default stable, handle advice supervision, and extract outcomes, acceptance, scope, constraints, blockers, route, repo, issue, and reusable-plan signals.', descVi: 'Tách các flag pipeline, quyết định mode theo thứ tự both > beta > stable mặc định, xử lý giám sát advice, rồi trích outcome, acceptance, scope, ràng buộc, blocker, route, repo, issue và tín hiệu plan có thể tái sử dụng.' },
    ...
    { number: 9, titleEn: 'Mark ready', titleVi: 'Đánh dấu sẵn sàng', descEn: 'Apply the ready to ship beta or ready to ship stable label to the source issue and PR, then remove stale ready to cook and in progress labels.', descVi: 'Gắn label ready to ship beta hoặc ready to ship stable cho issue nguồn và PR, rồi gỡ label ready to cook và in progress cũ.' },
  ...
  workflowModes: [
    { flag: 'default', modeEn: 'Stable PR readiness', modeVi: 'Sẵn sàng PR stable', research: 'Full plan gates', redTeam: 'Required', validation: 'Review and green checks', cookFlag: '/ak:ship official, no merge' },
    { flag: '--beta', modeEn: 'Beta PR readiness', modeVi: 'Sẵn sàng PR beta', research: 'Full plan gates', redTeam: 'Required', validation: 'Beta review and green checks', cookFlag: '/ak:ship beta, no merge' },
    { flag: '--ship', modeEn: 'Merge and converge', modeVi: 'Merge và đưa CI xanh', research: 'Full pipeline', redTeam: 'Required', validation: 'Post-merge CI loop', cookFlag: 'merge enabled; composes with beta' },
    { flag: '--both', modeEn: 'Beta then stable', modeVi: 'Beta rồi stable', research: 'Two-stage pipeline', redTeam: 'Per stage', validation: 'Beta CI before stable', cookFlag: 'supersedes beta; implies ship' },
  ],
  skillStack: [
  ...
  outputFlags: [
    ...
    { flag: '--beta', titleEn: 'Ship beta target', titleVi: 'Ship đích beta', descEn: 'Ship through the beta/dev target via /ak:ship beta and finish with the ready to ship beta label.', descVi: 'Ship qua đích beta/dev bằng /ak:ship beta và kết thúc bằng label ready to ship beta.', exampleCommand: '/ak:vibe --beta https://github.com/acme/app/issues/42' },
  ...
  promptExamples: [
    { labelEn: 'PR readiness from issue', labelVi: 'Đưa issue tới trạng thái sẵn PR', command: '/ak:vibe https://github.com/acme/app/issues/42', whenEn: 'A GitHub issue should be planned, implemented, reviewed, and labeled ready without automatic merge.', whenVi: 'Một GitHub issue cần được lập plan, triển khai, review và gắn ready nhưng chưa tự merge.', expectedEn: 'Creates an isolated worktree, validates and red-teams the plan, opens a reviewed stable PR, and applies ready to ship stable without merging.', expectedVi: 'Tạo worktree tách biệt, validate và red-team plan, mở PR stable đã review, rồi gắn ready to ship stable nhưng không merge.', recommended: true },
    { labelEn: 'Beta merge and CI convergence', labelVi: 'Merge beta và đưa CI xanh', command: '/ak:vibe --ship --beta https://github.com/acme/app/issues/42', whenEn: 'A GitHub issue should target beta/dev and continue through merge plus target-branch CI convergence.', whenVi: 'Một GitHub issue cần nhắm beta/dev và chạy tiếp qua merge cùng đưa CI branch đích về xanh.', expectedEn: 'Plans and implements in an isolated worktree, opens a beta PR, reviews/fixes/replies, merges, then watches or fixes beta/dev CI.', expectedVi: 'Lập plan và triển khai trong worktree tách biệt, mở PR beta, review/sửa/reply, merge, rồi theo dõi hoặc sửa CI beta/dev.' },
    { labelEn: 'Beta then stable', labelVi: 'Beta rồi stable', command: '/ak:vibe --both https://github.com/acme/app/issues/42', whenEn: 'The repo requires a beta/dev promotion before stable release.', whenVi: 'Repo yêu cầu qua beta/dev trước khi release stable.', expectedEn: 'Runs the beta PR through review, merge, and green beta/dev CI before starting stable PR review, merge, and stable CI convergence.', expectedVi: 'Chạy PR beta qua review, merge và CI beta/dev xanh trước khi bắt đầu review PR stable, merge và đưa CI stable về xanh.' },
    { labelEn: 'Advisory supervision', labelVi: 'Có giám sát tư vấn', command: '/ak:vibe --advice --ship https://github.com/acme/app/issues/42', whenEn: 'A high-stakes pipeline needs extra counsel without surrendering ownership.', whenVi: 'Pipeline rủi ro cao cần thêm tư vấn nhưng không chuyển quyền sở hữu.', expectedEn: 'Adds kongming counsel after key phases and after green PR checks, with comments on the PR and source issue while every gate still applies.', expectedVi: 'Thêm tư vấn kongming sau các phase chính và sau PR check xanh, comment lên PR và issue nguồn trong khi mọi gate vẫn áp dụng.' },
  ],
};
