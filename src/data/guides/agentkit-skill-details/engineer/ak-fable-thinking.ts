...
const data: SkillInfographic = {
  ...
  invocation: {
    ...
    arguments: [
      { token: "[task or question to reason through]", titleEn: "Reasoning target", titleVi: "Đối tượng suy luận", descEn: "Natural-language question, diagnosis, review, decision, or constrained-writing task. Include evidence, scope, authority boundary, success criteria, and any mechanical output rule; this guides reasoning and does not authorize edits or external action.", descVi: "Câu hỏi, chẩn đoán, review, quyết định hoặc bài viết có ràng buộc bằng ngôn ngữ tự nhiên. Nêu evidence, phạm vi, ranh giới quyền hạn, tiêu chí thành công và mọi quy tắc output có thể kiểm cơ học; phần này hướng dẫn suy luận và không cấp quyền sửa file hay hành động bên ngoài.", required: true, exampleCommand: "/ak:fable-thinking \"Assess whether the proposed retry change addresses Friday's checkout failures. Use the diff and incident logs, hold at least two hypotheses, and lead with GO, CAUTION, or STOP plus the weakest link.\"",
          exampleCommandVi: "/ak:fable-thinking \"Assess whether the proposed retry change addresses Friday's checkout failures. Use the diff and incident logs, hold at least two hypotheses, and lead with GO, CAUTION, or STOP plus the weakest link.\"" },
    ],
  },
  guardrails: [
  ...
  promptExamples: [
    { labelEn: "Root cause diagnosis", labelVi: "Chẩn đoán nguyên nhân gốc", command: "/ak:fable-thinking diagnose why this flaky test passes locally but fails in CI",
      commandVi: '/ak:fable-thinking chẩn đoán tại sao bài test flaky này pass ở local nhưng fail trong CI', whenEn: "Use when a familiar-looking failure needs mechanism, not a template fix.", whenVi: "Dùng khi lỗi trông quen cần cơ chế thật, không phải bản sửa theo khuôn.", expectedEn: "Runs the Floor, verifies load-bearing facts, compares multiple hypotheses, chooses discriminating checks, attacks the conclusion, and reports calibrated findings.", expectedVi: "Chạy Floor, xác minh facts chịu lực, so sánh nhiều giả thuyết, chọn phép kiểm phân biệt, phản biện kết luận và báo nhận định đúng mức.", recommended: true },
    { labelEn: "Strict surface constraint", labelVi: "Ràng buộc bề mặt chặt", command: "/ak:fable-thinking write a product tagline with exactly seven words and no letter e",
