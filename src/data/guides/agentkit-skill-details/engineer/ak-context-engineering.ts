  ...
  arguments: [
    {
      ...
      exampleCommand: '/ak:context-engineering "Our tool outputs dominate a long debugging session. Design a compaction policy that preserves errors, decisions, changed files, and next steps; do not write files."',
          exampleCommandVi: '/ak:context-engineering "Output của tool chiếm ưu thế trong một phiên debugging dài. Thiết kế compaction policy giữ lại errors, decisions, changed files và next steps; không ghi files."',
    },
  ],
};
...
const data: SkillInfographic = {
  ...
  promptExamples: [
    { labelEn: 'Usage warning', labelVi: 'Cảnh báo dung lượng', command: '/ak:context-engineering context window is at 82%; help compact without losing critical requirements, decisions, changed files, or next steps',
      commandVi: '/ak:context-engineering context window đang ở 82%; giúp compact mà không mất các requirement quan trọng, quyết định, file đã thay đổi, hoặc next steps', whenEn: 'A long session is near context limits.', whenVi: 'Khi phiên dài sắp chạm giới hạn context.', expectedEn: 'A measured compaction plan that names the pressure, preservation contract, reduction target, and probes for checking critical facts after compression.', expectedVi: 'Kế hoạch nén có đo lường, nêu áp lực, contract cần giữ, mục tiêu giảm token và probe để kiểm tra fact quan trọng sau khi nén.', recommended: true },
    { labelEn: 'Agent architecture', labelVi: 'Kiến trúc agent', command: '/ak:context-engineering design context isolation for a multi-agent review workflow with clear handoffs and cost boundaries',
      commandVi: '/ak:context-engineering thiết kế cô lập ngữ cảnh cho quy trình review đa agent với bàn giao rõ ràng và ranh giới chi phí', whenEn: 'A multi-agent system needs clean work partitioning.', whenVi: 'Khi hệ thống đa agent cần chia context sạch giữa các phần việc.', expectedEn: 'Guidance for splitting work across agents, passing only high-signal context, avoiding role-play-only isolation, and measuring tokens per task.', expectedVi: 'Hướng dẫn chia việc giữa agent, chỉ chuyển context tín hiệu cao, tránh cô lập chỉ để role-play và đo token theo mỗi nhiệm vụ.' },
    { labelEn: 'Memory design', labelVi: 'Thiết kế memory', command: '/ak:context-engineering design a cross-session memory approach for an LLM coding assistant without storing private user data',
      commandVi: '/ak:context-engineering thiết kế phương pháp bộ nhớ xuyên phiên cho trợ lý lập trình LLM mà không lưu trữ dữ liệu riêng tư của người dùng', whenEn: 'A product needs memory or retrieval without unsafe persistence.', whenVi: 'Khi sản phẩm cần memory hoặc retrieval mà không lưu trữ thiếu an toàn.', expectedEn: 'A bounded memory strategy covering what to write, what to retrieve just in time, privacy limits, and evaluation probes for future recall.', expectedVi: 'Chiến lược memory có giới hạn, gồm nội dung nên ghi, thứ cần truy xuất đúng lúc, giới hạn riêng tư và probe đánh giá recall sau này.' },
    { labelEn: 'Tool descriptions', labelVi: 'Mô tả tool', command: '/ak:context-engineering review these tool descriptions for token cost, clarity, when-to-use boundaries, inputs, and returns',
      commandVi: '/ak:context-engineering review các mô tả tool này về chi phí token, độ rõ ràng, ranh giới when-to-use, inputs và returns', whenEn: 'Tool definitions are bloated or agents choose the wrong tools.', whenVi: 'Khi mô tả tool phình to hoặc agent chọn sai tool.', expectedEn: 'A tool-design review using the four-question framework, with concrete trimming advice and checks for clearer selection boundaries.', expectedVi: 'Review thiết kế tool theo framework bốn câu hỏi, kèm đề xuất cắt gọn cụ thể và kiểm tra ranh giới chọn tool rõ hơn.' },
  ],
  invocation,
  specialOperations: [
