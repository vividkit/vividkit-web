...
const data: SkillInfographic = {
  ...
  kit: 'engineer',
  header: {
    titleEn: '/ak:agentize — Wrap existing code as CLI and MCP',
    titleVi: '/ak:agentize — Đóng gói mã hiện có thành CLI và MCP',
    taglineEn:
  ...
  processFlow: [
    ...
    { number: 6, titleEn: 'Wrap CLI and MCP', titleVi: 'Bọc CLI và MCP', descEn: 'Add scriptable CLI commands and official MCP tools with structured schemas, actionable errors, auth boundaries, and stdio/HTTP transport decisions.', descVi: 'Thêm lệnh CLI scriptable và tool MCP chính thức với schema có cấu trúc, lỗi chỉ cách khắc phục, ranh giới auth và quyết định transport stdio/HTTP.' },
  ...
  promptExamples: [
    { labelEn: 'Default both surfaces', labelVi: 'Mặc định cả hai bề mặt', command: '/ak:agentize packages/payments --both --auto', whenEn: 'Use when an existing module should become both an npm CLI and MCP server.', whenVi: 'Dùng khi module hiện có cần trở thành cả npm CLI và MCP server.', expectedEn: 'Tracks a plan, scouts the module, decides commands/tools, extracts core, wraps adapters, hardens, and packages.', expectedVi: 'Tạo kế hoạch, rà soát module, chốt lệnh/tool, tách core, bọc adapter, gia cố và đóng gói.', recommended: true },
    { labelEn: 'MCP with user decisions', labelVi: 'MCP có hỏi người dùng', command: '/ak:agentize "Expose report generation to agents" --mcp --ask', whenEn: 'Use when mutating/read-only scope, deployment target, or credential source needs human choice.', whenVi: 'Dùng khi phạm vi đọc/ghi, nơi deploy hoặc nguồn credential cần người dùng quyết định.', expectedEn: 'Scouts first, then asks targeted challenge questions before writing the decision record.', expectedVi: 'Rà soát trước, rồi hỏi các câu phản biện có trọng tâm trước khi viết bản ghi quyết định.' },
    { labelEn: 'Ultra decision pass', labelVi: 'Chế độ quyết định ultra', command: '/ak:agentize src/exporter --cli --ultra', whenEn: 'Use when capability selection and wrapper design deserve five independent decision records.', whenVi: 'Dùng khi việc chọn năng lực và thiết kế lớp bọc đáng có năm bản quyết định độc lập.', expectedEn: 'Phases 0-1 run once; analysis and decision fan out, then one verifier picks a winner.', expectedVi: 'Pha 0-1 chạy một lần; phân tích và quyết định tách nhánh, rồi một bộ xác minh chọn bản thắng.' },
    { labelEn: 'YAGNI cut', labelVi: 'Cắt scope bằng YAGNI', command: '/ak:agentize "Wrap the billing reconciliation module" --both --yagni', whenEn: 'Use when the wrapper should challenge unneeded capabilities rather than expose the full requested surface.', whenVi: 'Dùng khi lớp bọc cần phản biện năng lực không cần thiết thay vì mở toàn bộ bề mặt được yêu cầu.', expectedEn: 'The literal --yagni scope mode is passed downstream and only unnecessary scope is cut.', expectedVi: 'Cờ --yagni được truyền xuống dưới và chỉ cắt phần scope thật sự không cần.' },
  ],
  skillStack: [
