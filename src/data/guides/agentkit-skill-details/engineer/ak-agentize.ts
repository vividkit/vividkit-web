...
const data: SkillInfographic = {
  ...
  promptExamples: [
    { labelEn: 'Default both surfaces', labelVi: 'Mặc định cả hai bề mặt', command: '/ak:agentize packages/payments --both --auto',
      commandVi: '/ak:agentize packages/payments --both --auto', whenEn: 'Use when an existing module should become both an npm CLI and MCP server.', whenVi: 'Dùng khi module hiện có cần trở thành cả npm CLI và MCP server.', expectedEn: 'Creates tracked work, scouts real behavior, records the selected commands/tools, extracts shared core logic, wraps CLI and MCP adapters, then hardens tests, docs, CI, security checks, and the companion skill before package handoff.', expectedVi: 'Tạo kế hoạch có theo dõi, rà soát hành vi thật, ghi lại lệnh/tool đã chọn, tách core dùng chung, bọc adapter CLI và MCP, rồi gia cố test, tài liệu, CI, kiểm tra bảo mật và skill đi kèm trước khi bàn giao gói.', recommended: true },
    { labelEn: 'MCP with user decisions', labelVi: 'MCP có hỏi người dùng', command: '/ak:agentize "Expose report generation to agents" --mcp --ask',
      commandVi: '/ak:agentize "Mở khả năng tạo báo cáo cho các agent" --mcp --ask', whenEn: 'Use when mutating/read-only scope, deployment target, or credential source needs human choice.', whenVi: 'Dùng khi phạm vi đọc/ghi, nơi deploy hoặc nguồn credential cần người dùng quyết định.', expectedEn: 'Runs the mandatory track and scout phases first, then blocks in Phase 3 for scope, mutation, credential, deployment, package, ownership, and replacement-CLI answers before implementing the MCP surface.', expectedVi: 'Chạy các pha bắt buộc là lập kế hoạch và rà soát trước, rồi dừng ở pha 3 để hỏi về scope, thao tác ghi, credential, deploy, package, owner và CLI cần thay thế trước khi triển khai bề mặt MCP.' },
    { labelEn: 'Ultra decision pass', labelVi: 'Chế độ quyết định ultra', command: '/ak:agentize src/exporter --cli --ultra',
      commandVi: '/ak:agentize src/exporter --cli --ultra', whenEn: 'Use when capability selection and wrapper design deserve five independent decision records.', whenVi: 'Dùng khi việc chọn năng lực và thiết kế lớp bọc đáng có năm bản quyết định độc lập.', expectedEn: 'Runs tracking and scouting once, fans only the Agentization Map and decision record to five read-only candidates, lets one verifier select a winning record unchanged, then implements the CLI package from that winner.', expectedVi: 'Chạy lập kế hoạch và rà soát một lần, chỉ tách nhánh Agentization Map và decision record cho năm ứng viên chỉ đọc, để một verifier chọn bản thắng không chỉnh sửa, rồi triển khai gói CLI từ bản đó.' },
    { labelEn: 'YAGNI cut', labelVi: 'Cắt scope bằng YAGNI', command: '/ak:agentize "Wrap the billing reconciliation module" --both --yagni',
      commandVi: '/ak:agentize "Bọc module đối soát thanh toán" --both --yagni', whenEn: 'Use when the wrapper should challenge unneeded capabilities rather than expose the full requested surface.', whenVi: 'Dùng khi lớp bọc cần phản biện năng lực không cần thiết thay vì mở toàn bộ bề mặt được yêu cầu.', expectedEn: 'Scouts the module, builds an Agentization Map, challenges requested capabilities that do not serve the stated outcome, passes the literal --yagni flag downstream, and packages only the justified CLI/MCP surface.', expectedVi: 'Rà soát module, lập Agentization Map, phản biện những năng lực được yêu cầu nhưng không phục vụ mục tiêu đã nêu, truyền nguyên cờ --yagni xuống dưới và chỉ đóng gói bề mặt CLI/MCP có lý do.' },
  ],
  skillStack: [
  ...
  invocation: {
    syntax: '/ak:agentize [feature-or-module] [--both|--mcp|--cli] [--auto|--ask] [--ultra] [--advice] [--yagni]',
    arguments: [
      { token: '[feature-or-module]', titleEn: 'Feature or module', titleVi: 'Tính năng hoặc module', descEn: 'Existing codebase, feature, module, or subtree to expose. Name the target behavior to wrap; this skill is not for building a brand-new MCP server or raw package scaffold.', descVi: 'Codebase, tính năng, module hoặc thư mục con hiện có cần đưa ra ngoài. Nêu hành vi cần bọc; skill này không dùng để xây MCP server hoàn toàn mới hoặc scaffold package thô.', required: true, exampleCommand: '/ak:agentize packages/payments --both --auto',
          exampleCommandVi: '/ak:agentize packages/payments --both --auto' },
    ],
    options: [
      { token: '--both', titleEn: 'CLI and MCP', titleVi: 'CLI và MCP', descEn: 'Default output mode. Creates shared core logic plus CLI and MCP adapters from the existing capability.', descVi: 'Mode output mặc định. Tạo lõi dùng chung cùng adapter CLI và MCP từ năng lực hiện có.', exampleCommand: '/ak:agentize src/invoices --both',
          exampleCommandVi: '/ak:agentize src/invoices --both' },
      { token: '--mcp', titleEn: 'MCP only', titleVi: 'Chỉ MCP', descEn: 'Build only the MCP surface while preserving a core boundary for future CLI work.', descVi: 'Chỉ xây bề mặt MCP nhưng vẫn giữ ranh giới core để sau này có thể thêm CLI.', exampleCommand: '/ak:agentize "Expose report generation to agents" --mcp --ask',
          exampleCommandVi: '/ak:agentize "Expose generation báo cáo cho agents" --mcp --ask' },
      { token: '--cli', titleEn: 'CLI only', titleVi: 'Chỉ CLI', descEn: 'Build only the command-line surface while preserving a core boundary for future MCP work.', descVi: 'Chỉ xây bề mặt dòng lệnh nhưng vẫn giữ ranh giới core để sau này có thể thêm MCP.', exampleCommand: '/ak:agentize src/exporter --cli',
          exampleCommandVi: '/ak:agentize src/exporter --cli' },
      { token: '--auto', titleEn: 'Decide automatically', titleVi: 'Tự quyết định', descEn: 'Default interaction mode. Analyzes, decides, and implements without routine questions; asks only when a credential choice cannot be inferred safely.', descVi: 'Mode tương tác mặc định. Phân tích, quyết định và triển khai không hỏi thường lệ; chỉ hỏi khi không thể suy ra an toàn về credential.', exampleCommand: '/ak:agentize packages/payments --auto',
          exampleCommandVi: '/ak:agentize packages/payments --auto' },
      { token: '--ask', titleEn: 'Ask before building', titleVi: 'Hỏi trước khi xây', descEn: 'After analysis, pauses to challenge v1 scope, mutation boundaries, credentials, deployment target, package metadata, ownership, and any CLI replacement.', descVi: 'Sau khi phân tích, dừng để chất vấn scope v1, ranh giới thao tác ghi, credential, nơi deploy, metadata package, owner và CLI cần thay thế nếu có.', exampleCommand: '/ak:agentize src/invoices --both --ask',
          exampleCommandVi: '/ak:agentize src/invoices --both --ask' },
      { token: '--ultra', titleEn: 'Five decision candidates', titleVi: 'Năm ứng viên quyết định', descEn: 'Runs tracking and scouting once, then fans out only the analysis and decision phases to five read-only candidates before one verifier chooses the record to implement.', descVi: 'Chạy lập kế hoạch và rà soát một lần, rồi chỉ tách nhánh phân tích và quyết định cho năm ứng viên chỉ đọc trước khi một verifier chọn bản để triển khai.', exampleCommand: '/ak:agentize src/exporter --cli --ultra',
          exampleCommandVi: '/ak:agentize src/exporter --cli --ultra' },
      { token: '--advice', titleEn: 'Advisory supervision', titleVi: 'Giám sát cố vấn', descEn: 'Adds advisory-only kongming checkpoints after scouting and analysis, before the decision record, before package handoff, and when stuck. Advice cannot edit code or skip gates.', descVi: 'Thêm checkpoint tư vấn-only với kongming sau rà soát và phân tích, trước bản ghi quyết định, trước bàn giao package và khi bị kẹt. Advice không sửa code hoặc bỏ qua gate.', exampleCommand: '/ak:agentize packages/payments --both --advice',
          exampleCommandVi: '/ak:agentize packages/payments --both --advice' },
      { token: '--yagni', titleEn: 'Cut unnecessary scope', titleVi: 'Cắt scope thừa', descEn: 'Challenges capabilities that do not serve the stated outcome and passes the literal flag to downstream skills and delegated work.', descVi: 'Chất vấn những năng lực không phục vụ outcome đã nêu và truyền nguyên cờ này xuống skill cùng việc được giao bên dưới.', exampleCommand: '/ak:agentize "Wrap the billing reconciliation module" --both --yagni',
          exampleCommandVi: '/ak:agentize "Wrap module đối soát billing" --both --yagni' },
    ],
  },
};
