...
const data: SkillInfographic = {
  ...
  invocation: {
    ...
    arguments: [
      { token: '[task]', titleEn: 'MCP task', titleVi: 'Task MCP', descEn: 'Natural-language request to discover MCP tools, inspect schemas, or call an existing server tool. Include the server name, read-only boundary, or approved external write when relevant.', descVi: 'Yêu cầu bằng ngôn ngữ tự nhiên để khám phá MCP tool, kiểm schema hoặc gọi tool của server hiện có. Nêu tên server, ranh giới read-only hoặc external write đã duyệt khi phù hợp.', required: true, exampleCommand: '/ak:use-mcp "List the issue tools available from the registered project server; do not call a mutating tool"',
          exampleCommandVi: '/ak:use-mcp "List các công cụ issue có sẵn từ project server đã đăng ký; không gọi công cụ thay đổi dữ liệu"' },
    ],
  },
  specialOperations: [
  ...
  promptExamples: [
    { labelEn: 'Discover available tools', labelVi: 'Tìm tool đang có', command: '/ak:use-mcp list the MCP tools available for the current runtime without calling mutating tools',
      commandVi: '/ak:use-mcp list các công cụ MCP có sẵn cho runtime hiện tại mà không gọi các công cụ thay đổi trạng thái', whenEn: 'You need capability discovery before choosing a server/tool.', whenVi: 'Cần biết runtime hiện tại có capability nào trước khi chọn server/tool.', expectedEn: 'Inventories the active runtime MCP surface first, keeps discovery read-only, then reports visible servers/tools or the visibility boundary that requires direct scripts.', expectedVi: 'Kiểm kê bề mặt MCP của runtime hiện tại trước, giữ discovery read-only, rồi báo server/tool thấy được hoặc ranh giới visibility cần direct script.', recommended: true },
    { labelEn: 'Call a known tool', labelVi: 'Gọi tool đã biết', command: '/ak:use-mcp call the existing GitHub MCP tool to read issue 42 without changing the issue',
      commandVi: '/ak:use-mcp gọi công cụ GitHub MCP hiện có để đọc issue 42 mà không thay đổi issue', whenEn: 'A server/tool is already registered and the task has a concrete read target.', whenVi: 'Server/tool đã đăng ký sẵn và task có mục tiêu đọc cụ thể.', expectedEn: 'Selects the smallest matching GitHub tool, validates its live input schema, classifies the call as read-only, executes it, and reports the bounded arguments and result.', expectedVi: 'Chọn GitHub tool phù hợp nhỏ nhất, kiểm schema input live, phân loại call là read-only, chạy tool, rồi báo argument đã giới hạn và kết quả.' },
    { labelEn: 'Use scripted fallback', labelVi: 'Dùng direct script', command: '/ak:use-mcp discover tools from .claude/.mcp.json for this project and persist the catalog',
      commandVi: '/ak:use-mcp khám phá các công cụ từ .claude/.mcp.json cho dự án này và lưu catalog', whenEn: 'The runtime does not expose a configured stdio server but the project MCP config exists.', whenVi: 'Runtime không hiện stdio server đã cấu hình nhưng project có MCP config.', expectedEn: 'Uses the bundled scripts client for the project configuration, runs list-tools/list-prompts/list-resources as needed, and persists assets/tools.json for schema review before any call.', expectedVi: 'Dùng client script đi kèm cho cấu hình project, chạy list-tools/list-prompts/list-resources khi cần, và lưu assets/tools.json để xem schema trước mọi call.' },
    { labelEn: 'Guard Chrome profile access', labelVi: 'Giữ đúng profile Chrome', command: '/ak:use-mcp inspect the Chrome DevTools MCP options for my logged-in browser profile before using cookies',
      commandVi: '/ak:use-mcp kiểm tra các tùy chọn Chrome DevTools MCP cho hồ sơ trình duyệt đã đăng nhập của tôi trước khi dùng cookie', whenEn: 'An MCP-backed browser task depends on real cookies or an exact Chrome profile.', whenVi: 'Task browser qua MCP phụ thuộc cookie thật hoặc đúng profile Chrome.', expectedEn: 'Recognizes the DevTools MCP profile boundary, routes profile setup through ak:chrome-profile first, then uses only the returned visible MCP/browser capability for scoped work.', expectedVi: 'Nhận diện ranh giới profile của DevTools MCP, chuyển phần thiết lập profile qua ak:chrome-profile trước, rồi chỉ dùng capability MCP/browser đang thấy cho việc đúng scope.' },
  ],
};
