...
const data: SkillInfographic = {
  ...
  invocation: {
    syntax: '/ak:deep-swe <OpenRouter model slug> [benchmark request]',
    arguments: [
      { token: '<OpenRouter model slug>', titleEn: 'Model slug', titleVi: 'Model slug', descEn: 'Exact OpenRouter vendor/model identifier to evaluate on DeepSWE. The Skill verifies it before spending tokens; it is not an AgentKit-owned catalog entry.', descVi: 'Identifier vendor/model chính xác trên OpenRouter cần đánh giá bằng DeepSWE. Skill xác minh trước khi tiêu token; đây không phải mục trong catalog do AgentKit sở hữu.', required: true, exampleCommand: '/ak:deep-swe openai/gpt-4o-mini',
          exampleCommandVi: '/ak:deep-swe openai/gpt-4o-mini' },
      { token: '[benchmark request]', titleEn: 'Benchmark request', titleVi: 'Yêu cầu benchmark', descEn: 'Optional natural-language boundary such as one named smoke task, a deterministic subset, or verification of an existing Pier job. It does not authorize a full corpus run or result submission by default.', descVi: 'Ranh giới tùy chọn bằng ngôn ngữ tự nhiên, ví dụ một task smoke cụ thể, subset xác định hoặc xác minh job Pier đã có. Mặc định không cho phép chạy toàn bộ corpus hoặc nộp kết quả.', exampleCommand: '/ak:deep-swe openai/gpt-4o-mini "Run one named DeepSWE task as a smoke test. Stop after the task and do not submit results."',
          exampleCommandVi: '/ak:deep-swe openai/gpt-4o-mini "Chạy một task DeepSWE được đặt tên như smoke test. Stop sau task và không submit kết quả."' },
    ],
  },
  promptExamples: [
    { labelEn: 'Smoke one task', labelVi: 'Smoke một task', command: '/ak:deep-swe openai/gpt-4o-mini "Run one named DeepSWE task as a smoke test. Show the exact Pier command and estimated spend exposure before execution. Stop after the task and do not submit results."',
      commandVi: '/ak:deep-swe openai/gpt-4o-mini "Chạy một task DeepSWE có tên như smoke test. Hiện đúng lệnh Pier và ước tính spend exposure trước khi thực thi. Dừng sau task và không submit kết quả."', whenEn: 'You want to score a specific OpenRouter model slug on DeepSWE without approving a larger run yet.', whenVi: 'Khi muốn chấm một OpenRouter model slug cụ thể trên DeepSWE nhưng chưa duyệt run lớn hơn.', expectedEn: 'Checks uv, git, Docker, the daemon, and key presence; validates Pier help and runs only one named task before reporting local evidence.', expectedVi: 'Kiểm uv, git, Docker, daemon và key; xác minh help của Pier rồi chỉ chạy một task cụ thể trước khi báo bằng chứng local.', recommended: true },
    { labelEn: 'Deterministic subset', labelVi: 'Subset xác định', command: '/ak:deep-swe openai/gpt-4o-mini "After a successful smoke task, prepare a 10-task deterministic subset with --n-tasks 10 and --sample-seed 0 only if current pier run --help confirms those flags."',
      commandVi: '/ak:deep-swe openai/gpt-4o-mini "Sau khi smoke task thành công, chuẩn bị subset 10 task mang tính deterministic với --n-tasks 10 và --sample-seed 0 chỉ khi pier run --help hiện tại xác nhận các flag đó."', whenEn: 'You approved expansion from smoke evidence to a small fixed sample.', whenVi: 'Khi đã duyệt mở rộng từ bằng chứng smoke sang một sample nhỏ cố định.', expectedEn: 'Rechecks the installed Pier flag surface, presents the bounded subset command and spend exposure, then reports task count, score or reward, cost if available, and blockers.', expectedVi: 'Kiểm lại flag của Pier đã cài, trình bày lệnh subset có giới hạn và rủi ro chi phí, rồi báo số task, điểm hoặc reward, cost nếu có và blocker.' },
    { labelEn: 'Score verification', labelVi: 'Xác minh điểm', command: '/ak:deep-swe verify the Pier job result for openai/gpt-4o-mini',
      commandVi: '/ak:deep-swe xác minh kết quả job Pier cho openai/gpt-4o-mini', whenEn: 'You need to verify or summarize an existing DeepSWE Pier benchmark result.', whenVi: 'Khi cần xác minh hoặc tóm tắt một kết quả benchmark DeepSWE Pier đã có.', expectedEn: 'Inspects the generated job directory with available Pier view, analyze, or critique commands and reports command, version, task count, model slug, score or reward, cost, blockers, and submission status.', expectedVi: 'Kiểm job directory bằng các lệnh Pier view, analyze hoặc critique nếu có rồi báo lệnh, version, số task, model slug, điểm hoặc reward, cost, blocker và trạng thái submission.' },
  ],
