  ...
  arguments: [
    {
      ...
      exampleCommand: '/ak:document-skills "Create board-report.xlsx from approved-metrics.csv with formulas for totals and growth, preserve source precision, add a Sources sheet, recalculate with LibreOffice, scan every formula result for errors, and save to ./deliverables without changing the CSV."',
          exampleCommandVi: '/ak:document-skills "Create board-report.xlsx từ approved-metrics.csv với formulas cho totals và growth, giữ nguyên độ chính xác source, add một sheet Sources, tính lại bằng LibreOffice, scan mọi kết quả formula để tìm errors, và save vào ./deliverables without changing the CSV."',
    },
  ],
};
...
const data: SkillInfographic = {
  ...
  promptExamples: [
    { labelEn: "Create spreadsheet", labelVi: "Tạo spreadsheet", command: "/ak:document-skills \"Create board-report.xlsx from approved-metrics.csv with formulas for totals and growth, add a Sources sheet, and verify the workbook before returning the output path.\"",
      commandVi: '/ak:document-skills "Tạo board-report.xlsx từ approved-metrics.csv với công thức cho tổng và tăng trưởng, thêm sheet Sources, và xác minh workbook trước khi trả về đường dẫn output."', whenEn: "Use when tabular source data needs to become a real XLSX workbook rather than a prose table.", whenVi: "Dùng khi dữ liệu dạng bảng cần thành workbook XLSX thật thay vì bảng văn bản.", expectedEn: "Classifies the request as xlsx work, uses the bundled document material for spreadsheets, creates the workbook artifact, and inspects formulas, sheets, and cell placement before delivery.", expectedVi: "Phân loại yêu cầu là công việc xlsx, dùng tài liệu document đi kèm cho spreadsheet, tạo workbook artifact và kiểm tra formula, sheet cùng vị trí ô trước khi bàn giao.", recommended: true },
    { labelEn: "Edit Word document", labelVi: "Sửa tài liệu Word", command: "/ak:document-skills \"Update proposal.docx with the approved executive summary, preserve existing headings and tables, and return a verified edited copy instead of overwriting the original.\"",
      commandVi: '/ak:document-skills "Cập nhật proposal.docx bằng tóm tắt điều hành đã được phê duyệt, giữ nguyên headings và tables hiện có, và trả về bản sao đã chỉnh sửa được xác minh thay vì ghi đè bản gốc."', whenEn: "Use when a DOCX needs targeted content or structure edits while retaining Office document semantics.", whenVi: "Dùng khi DOCX cần sửa nội dung hoặc cấu trúc có mục tiêu mà vẫn giữ ngữ nghĩa tài liệu Office.", expectedEn: "Routes the task to docx handling, inspects the existing document structure, applies the requested edits to a separate artifact, and verifies that headings, tables, and requested content survived.", expectedVi: "Route tác vụ sang xử lý docx, kiểm tra cấu trúc tài liệu hiện có, áp dụng chỉnh sửa vào artifact riêng và xác minh heading, bảng cùng nội dung yêu cầu vẫn được giữ." },
    { labelEn: "Read PDF form", labelVi: "Đọc form PDF", command: "/ak:document-skills \"Read intake-form.pdf, extract its form fields and table content, and report any extraction limits or fields that require visual inspection.\"",
      commandVi: '/ak:document-skills "Đọc intake-form.pdf, trích xuất các trường form và nội dung table, và báo cáo mọi giới hạn trích xuất hoặc các trường cần kiểm tra trực quan."', whenEn: "Use when a PDF needs structured extraction, form-field reading, or artifact inspection instead of a plain summary.", whenVi: "Dùng khi PDF cần trích xuất có cấu trúc, đọc field biểu mẫu hoặc kiểm tra artifact thay vì chỉ tóm tắt.", expectedEn: "Uses the PDF route, inspects the file for structured content and form fields, extracts the requested data, and names any conversion or inspection limits in the result.", expectedVi: "Dùng route PDF, kiểm tra file để tìm nội dung có cấu trúc và field biểu mẫu, trích dữ liệu được yêu cầu rồi nêu mọi giới hạn chuyển đổi hoặc kiểm tra trong kết quả." },
    { labelEn: "Build slide deck", labelVi: "Tạo slide deck", command: "/ak:document-skills \"Create a 10-slide project-update.pptx from outline.md, use the supplied template, keep speaker notes where provided, and verify the deck layout before delivery.\"",
      commandVi: '/ak:document-skills "Tạo project-update.pptx 10 slide từ outline.md, sử dụng template được cung cấp, giữ speaker notes nếu có, và xác minh layout của deck trước khi giao."', whenEn: "Use when a PowerPoint deliverable must be created or updated as a PPTX artifact with slide structure.", whenVi: "Dùng khi cần tạo hoặc cập nhật deliverable PowerPoint dưới dạng artifact PPTX có cấu trúc slide.", expectedEn: "Routes the request to pptx handling, uses the bundled document guidance for slides, creates the deck artifact, and reviews slide structure, notes, and layout surface before returning it.", expectedVi: "Route yêu cầu sang xử lý pptx, dùng hướng dẫn document đi kèm cho slide, tạo deck artifact và review cấu trúc slide, note cùng bề mặt layout trước khi trả file." },
  ],
};
