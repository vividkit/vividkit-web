...
const data: SkillInfographic = {
  ...
  invocation: {
    syntax: '/ak:payment-integration [provider] [task]',
    arguments: [
      { token: '[provider]', titleEn: 'Provider', titleVi: 'Provider', descEn: 'Names the payment provider or an intentional multi-provider boundary, such as SePay, Polar, Stripe, or multi-provider orders.', descVi: 'Nêu provider thanh toán hoặc ranh giới multi-provider có chủ đích, như SePay, Polar, Stripe hoặc order đa provider.', required: false, exampleCommand: '/ak:payment-integration stripe Add a sandbox subscription checkout',
          exampleCommandVi: '/ak:payment-integration stripe Add một sandbox subscription checkout' },
      { token: '[task]', titleEn: 'Payment task', titleVi: 'Tác vụ thanh toán', descEn: 'Describes the checkout, subscription, webhook, VietQR, order, refund, test, and authority contract to design or implement; do not include secrets.', descVi: 'Mô tả contract thiết kế hoặc triển khai checkout, subscription, webhook, VietQR, order, refund, test và quyền hạn; không đưa secret.', required: false, exampleCommand: '/ak:payment-integration "stripe Add a sandbox subscription checkout and verify webhook signatures on the raw body"',
          exampleCommandVi: '/ak:payment-integration "stripe Add một sandbox subscription checkout và xác minh webhook signatures trên raw body"' },
    ],
  },
  processFlow: [
  ...
  promptExamples: [
    { labelEn: 'SePay QR', labelVi: 'QR SePay', command: '/ak:payment-integration SePay VietQR checkout for VND orders',
      commandVi: '/ak:payment-integration SePay VietQR checkout cho đơn hàng VND', whenEn: 'Use for Vietnamese bank-transfer payments and QR codes.', whenVi: 'Dùng cho thanh toán chuyển khoản ngân hàng Việt Nam và QR code.', expectedEn: 'Returns SePay references, a VietQR checkout flow, webhook verification steps, and durable order-state plan.', expectedVi: 'Trả về tài liệu SePay, luồng checkout VietQR, bước xác minh webhook và kế hoạch trạng thái đơn hàng bền vững.', recommended: true },
    { labelEn: 'Polar SaaS', labelVi: 'SaaS với Polar', command: '/ak:payment-integration Polar subscriptions with GitHub benefits',
      commandVi: '/ak:payment-integration đăng ký Polar với quyền lợi GitHub', whenEn: 'Use for SaaS subscriptions and automated benefit delivery.', whenVi: 'Dùng cho subscription SaaS và cấp benefit tự động.', expectedEn: 'Designs Polar checkout, subscription lifecycle handling, benefit delivery, webhook verification, and renewal-state mapping.', expectedVi: 'Thiết kế checkout Polar, xử lý vòng đời subscription, cấp benefit, xác minh webhook và mapping trạng thái gia hạn.' },
    { labelEn: 'Stripe enterprise', labelVi: 'Stripe enterprise', command: '/ak:payment-integration Stripe Connect marketplace payouts',
      commandVi: '/ak:payment-integration payout marketplace Stripe Connect', whenEn: 'Use for complex global payment infrastructure or Connect platforms.', whenVi: 'Dùng cho hạ tầng thanh toán global phức tạp hoặc nền tảng Connect.', expectedEn: 'Provides a Stripe integration path covering checkout, Connect payouts, local webhook testing, and reconciliation.', expectedVi: 'Đưa hướng tích hợp Stripe bao phủ checkout, payout Connect, kiểm thử webhook cục bộ và đối soát.' },
  ],
