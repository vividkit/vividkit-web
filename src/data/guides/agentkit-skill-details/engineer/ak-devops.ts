  ...
  arguments: [
    {
      ...
      exampleCommand: "/ak:devops cloudflare \"review this Worker staging config\"",
          exampleCommandVi: '/ak:devops cloudflare "review this Worker staging config"',
    },
    {
      token: "[task]",
      titleEn: "Task",
      ...
      exampleCommand: "/ak:devops kubernetes \"review manifests for RBAC, resources, rollout checks, and rollback commands\"",
          exampleCommandVi: '/ak:devops kubernetes "review manifests for RBAC, resources, rollout checks, and rollback commands"',
    },
  ],
};
...
const data: SkillInfographic = {
  ...
  promptExamples: [
    { labelEn: "Cloudflare review", labelVi: "Rà soát Cloudflare", command: "/ak:devops cloudflare \"review this Worker staging config and prepare deploy, dry-run, and rollback steps\"",
      commandVi: '/ak:devops cloudflare "xem xét cấu hình Worker staging này và chuẩn bị các bước deploy, dry-run và rollback"', whenEn: "Cloudflare Workers, Pages, R2, D1, KV, or Browser Rendering is central to the infrastructure task.", whenVi: "Cloudflare Workers, Pages, R2, D1, KV hoặc Browser Rendering là trọng tâm của việc hạ tầng.", expectedEn: "Maps the target to the right Cloudflare surface, loads the relevant references, prepares wrangler-oriented commands, and reports bindings, routes, limits, cost, and rollback evidence.", expectedVi: "Gắn đích với đúng phần Cloudflare, mở tài liệu liên quan, chuẩn bị lệnh theo wrangler và báo cáo binding, route, limit, chi phí cùng bằng chứng rollback.", recommended: true },
    { labelEn: "Docker review", labelVi: "Rà soát Docker", command: "/ak:devops docker \"analyze this Dockerfile and propose build/run improvements\"",
      commandVi: '/ak:devops docker "phân tích Dockerfile này và đề xuất cải tiến build/run"', whenEn: "A service needs Dockerfile or Docker Compose review before local development or deployment.", whenVi: "Một service cần rà soát Dockerfile hoặc Docker Compose trước khi phát triển local hay deploy.", expectedEn: "Uses Docker references and the analyzer-oriented workflow to check build context, ports, volumes, health checks, multi-stage builds, non-root runtime, and secret handling.", expectedVi: "Dùng tài liệu Docker và workflow kiểu analyzer để kiểm tra build context, port, volume, health check, multi-stage build, runtime không chạy root và cách xử lý secret." },
    { labelEn: "GCP deploy plan", labelVi: "Kế hoạch deploy GCP", command: "/ak:devops gcp \"prepare a Cloud Run deployment plan for this container image\"",
      commandVi: '/ak:devops gcp "chuẩn bị kế hoạch triển khai Cloud Run cho container image này"', whenEn: "Google Cloud Run, GKE, Cloud SQL, IAM, region, quota, or gcloud operations define the release risk.", whenVi: "Google Cloud Run, GKE, Cloud SQL, IAM, region, quota hoặc thao tác gcloud quyết định rủi ro phát hành.", expectedEn: "Loads Google Cloud and gcloud guidance, resolves project, region, service account, APIs, cost and rollback boundaries, then proposes verifiable Cloud Run or GKE commands.", expectedVi: "Mở hướng dẫn Google Cloud và gcloud, phân giải project, region, service account, API, ranh giới chi phí và rollback, rồi đề xuất lệnh Cloud Run hoặc GKE có thể xác minh." },
    { labelEn: "Kubernetes audit", labelVi: "Rà soát Kubernetes", command: "/ak:devops kubernetes \"review manifests for RBAC, network policies, resources, and Helm rollout risk\"",
      commandVi: '/ak:devops kubernetes "xem xét manifests về RBAC, network policies, resources và rủi ro rollout Helm"', whenEn: "A Kubernetes cluster, manifest set, Helm release, RBAC policy, or GitOps workflow needs operational review.", whenVi: "Một cluster Kubernetes, bộ manifest, Helm release, policy RBAC hoặc workflow GitOps cần rà soát vận hành.", expectedEn: "Uses upstream Kubernetes and Helm references, checks context and namespace, then reviews RBAC, secrets, network policies, resources, rollout checks, and rollback commands.", expectedVi: "Dùng tài liệu upstream Kubernetes và Helm, kiểm tra context và namespace, rồi rà soát RBAC, secret, network policy, resource, kiểm tra rollout và lệnh rollback." },
  ],
};
