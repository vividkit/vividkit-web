  ...
  arguments: [
    {
      ...
      exampleCommand:
        '/ak:ai-multimodal ./assets/receipt.png "Extract merchant, date, currency, total, and line items to ./artifacts/receipt.json"',
          exampleCommandVi: '/ak:ai-multimodal ./assets/receipt.png "Trích xuất người bán, ngày, tiền tệ, tổng tiền và các mục dòng vào ./artifacts/receipt.json"',
    },
    {
      token: '[prompt]',
      titleEn: 'Task prompt',
      ...
      exampleCommand:
        '/ak:ai-multimodal "Generate a 15-second product demo video to ./artifacts/demo.mp4 after resolving a currently available provider model, duration, resolution, aspect ratio, pricing, and retention"',
          exampleCommandVi: '/ak:ai-multimodal "Tạo video demo sản phẩm 15 giây vào ./artifacts/demo.mp4 sau khi xác định model nhà cung cấp hiện có, thời lượng, độ phân giải, tỷ lệ khung hình, giá cả và thời gian lưu giữ"',
    },
  ],
};
...
const data: SkillInfographic = {
  ...
  promptExamples: [
    { labelEn: 'Extract receipt data', labelVi: 'Trích xuất dữ liệu hóa đơn', command: '/ak:ai-multimodal ./assets/receipt.png "Extract merchant, date, currency, total, and line items to ./artifacts/receipt.json; separate unreadable fields from inferred values"',
      commandVi: '/ak:ai-multimodal ./assets/receipt.png "Trích xuất người bán, ngày, tiền tệ, tổng và các mục dòng vào ./artifacts/receipt.json; tách các trường không đọc được khỏi các giá trị suy luận"', whenEn: 'Use for OCR, visual understanding, design extraction, or structured extraction from media.', whenVi: 'Dùng cho OCR, hiểu nội dung hình ảnh, trích xuất thiết kế hoặc trích xuất có cấu trúc từ media.', expectedEn: 'Verifies Multix setup, selects the Gemini analyze or extract command family, uploads only the named file, and returns schema-shaped output with uncertainty called out.', expectedVi: 'Xác minh setup Multix, chọn họ lệnh Gemini analyze hoặc extract, chỉ upload file đã nêu và trả output theo schema kèm phần không chắc chắn.', recommended: true },
    { labelEn: 'Transcribe interview video', labelVi: 'Chép lời video phỏng vấn', command: '/ak:ai-multimodal ./media/interview.mp4 "Generate a Markdown transcript with metadata, chunk status, and timestamped lines to ./artifacts/interview-transcript.md"',
      commandVi: '/ak:ai-multimodal ./media/interview.mp4 "Tạo bản ghi Markdown với metadata, trạng thái chunk và các dòng có dấu thời gian vào ./artifacts/interview-transcript.md"', whenEn: 'Use for audio or video transcription when timestamps and chunk status matter.', whenVi: 'Dùng cho chép lời audio hoặc video khi timestamp và trạng thái chunk là quan trọng.', expectedEn: 'Runs the Gemini transcribe path through the npm-latest CLI, preserves timestamped Markdown evidence, and reports missing keys, FFmpeg, quota, or provider errors as blockers.', expectedVi: 'Chạy đường Gemini transcribe qua CLI npm-latest, giữ bằng chứng Markdown có timestamp và báo thiếu key, FFmpeg, quota hoặc lỗi provider như blocker.' },
    { labelEn: 'Convert a document', labelVi: 'Chuyển đổi tài liệu', command: '/ak:ai-multimodal ./docs/report.pdf "Convert this supported document to Markdown at ./artifacts/report.md and include any conversion warnings"',
      commandVi: '/ak:ai-multimodal ./docs/report.pdf "Chuyển đổi tài liệu được hỗ trợ này sang Markdown tại ./artifacts/report.md và bao gồm mọi cảnh báo chuyển đổi"', whenEn: 'Use when the artifact is a document that should become Markdown through Multix doc convert.', whenVi: 'Dùng khi hiện vật là document cần chuyển thành Markdown qua Multix doc convert.', expectedEn: 'Checks the npm-latest Multix document command, writes the requested Markdown file, and includes conversion warnings or unsupported-format limits instead of inventing missing content.', expectedVi: 'Kiểm tra lệnh document của Multix npm-latest, ghi file Markdown được yêu cầu và nêu warning chuyển đổi hoặc giới hạn format không hỗ trợ thay vì bịa nội dung thiếu.' },
    { labelEn: 'Generate provider-backed media', labelVi: 'Tạo media qua provider', command: '/ak:ai-multimodal "Generate a 15-second product demo video to ./artifacts/demo.mp4 after resolving a currently available provider model, duration, resolution, aspect ratio, pricing, and retention"',
      commandVi: '/ak:ai-multimodal "Tạo video demo sản phẩm 15 giây vào ./artifacts/demo.mp4 sau khi xác định model nhà cung cấp hiện có, thời lượng, độ phân giải, tỷ lệ khung hình, giá cả và thời gian lưu giữ"', whenEn: 'Use when image, video, speech, or music generation needs live provider and model capability checks.', whenVi: 'Dùng khi tạo ảnh, video, speech hoặc music cần kiểm tra live năng lực provider và model.', expectedEn: 'Inspects current Multix help and provider catalogs before choosing explicit generation controls, records the selected model, and reviews the generated artifact against the brief.', expectedVi: 'Xem help Multix và catalog provider hiện tại trước khi chọn tham số tạo nội dung rõ ràng, ghi lại model đã chọn và review artifact theo brief.' },
  ],
  invocation,
  specialOperations: [
